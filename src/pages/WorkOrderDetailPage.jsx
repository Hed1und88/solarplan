...
import { WORK_ORDER_STATUSES, WORK_ORDER_TYPES, calculateWorkOrder, deleteWorkOrder, getWorkOrder, saveWorkOrder, uploadWorkOrderFile } from '@/lib/workOrderStore';
...
function Lines({ title, field, lines, setOrder, receipts = false }) {
  ...
  const add = () => setOrder(current => ({ ...current, [field]: [...(current[field] || []), { id: newId(field), description: '', quantity: 1, unit: 'st', unitPrice: 0, vatRate: current.vatRate || 25, privateExpense: receipts, reimbursable: true }] }));
  ...
  return <Section title={title} action={<Button type="button" size="sm" variant="outline" onClick={add}><Plus className="mr-1 h-4 w-4" />Lägg till</Button>}>
    <div className="space-y-3">{(lines || []).map(line => <div key={line.id} className="rounded-xl border border-slate-100 bg-slate-50 p-3">
      ...
      {receipts && <div className="mt-2 flex items-center gap-3"><label className="cursor-pointer rounded-lg border bg-white px-3 py-2 text-xs"><Upload className="mr-1 inline h-4 w-4" />{line.receiptName || 'Fota/ladda upp kvitto'}<input className="hidden" type="file" accept="image/*,application/pdf" onChange={event => upload(line, event.target.files?.[0])} /></label>{line.receiptUrl ? <a className="text-xs text-blue-600" href={line.receiptUrl} target="_blank" rel="noreferrer">Öppna kvitto</a> : <span className="text-xs text-red-600">Kvitto krävs</span>}</div>}
    </div>)}</div>
  </Section>;
}
...
export default function WorkOrderDetailPage() {
  ...
  const save = nextStatus => {
    if (nextStatus === 'ready_to_invoice' && (order.expenseLines || []).some(line => line.privateExpense && !line.receiptUrl)) { setMessage('Privata utlägg måste ha kvitto innan fakturering.'); return; }
    const saved = saveWorkOrder({ ...order, ...(nextStatus ? { status: nextStatus } : {}) });
    setOrder(saved); setMessage(nextStatus ? 'Klar för fakturering.' : 'Sparad.');
  };
