...
import { ArrowLeft, Sun, Cable, Battery, ShoppingCart, BarChart2, Wrench, GitBranch, Save, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export default function ProjectDetail() {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data) => saveProjectPatch(base44, workingProject || serverProject, data),
    onMutate: async (data) => {
      setSaveStatus({ state: 'saving', message: 'Sparar...' });
    ...
    onError: (error, _data, context) => {
      ...
      setSaveStatus({ state: 'error', message: error?.message || 'Kunde inte spara till servern. Lokal backup är kvar.' });
    },
    onSuccess: (updatedProject) => {
      ...
      setSaveStatus({ state: 'saved', message: `Sparat ${new Date().toLocaleTimeString('sv-SE')}` });
    },
  });

  const saveProject = async (data = {}) => updateMutation.mutateAsync(data);
    ...
    <div className="p-6 lg:p-10 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          {saveStatus.message && <div className={`mt-2 inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-medium ${saveStatus.state === 'error' ? 'bg-amber-50 text-amber-800' : saveStatus.state === 'saved' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-700'}`}>{saveStatus.state === 'error' ? <AlertTriangle className="h-3 w-3" /> : saveStatus.state === 'saved' ? <CheckCircle2 className="h-3 w-3" /> : <Save className="h-3 w-3" />}{saveStatus.message}</div>}
        </div>
      ...
      <Tabs defaultValue="panels" className="space-y-4">
        ...
        <TabsContent value="panels" className="space-y-4"><SolarRoofPlannerV2 project={project} onUpdate={saveProject} /></TabsContent>
        <TabsContent value="strings"><StringMarkingTabV2 project={project} onUpdate={saveProject} selectedProduct={(() => { try { const planner = JSON.parse(project.solar_roof_planner_data || '{}'); const pid = planner?.roofs?.find(roof => roof.panelProductId)?.panelProductId; if (pid) return products.find(p => p.id === pid) || null; } catch {} return products.find(p => project.selected_products?.some(sp => sp.product_id === p.id) && p.category === 'solpanel') || null; })()} /></TabsContent>
