  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all
                  ${active ? 'bg-primary text-white shadow-md shadow-primary/30' : 'text-sidebar-foreground hover:bg-sidebar-accent hover:text-white'}`}
              >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden lg:ml-64">
        ...
        <header
          className="lg:hidden flex items-center justify-between bg-card border-b border-border"
          style={{ paddingTop: 'env(safe-area-inset-top)', paddingLeft: 'max(16px, env(safe-area-inset-left))', paddingRight: 'max(16px, env(safe-area-inset-right))', paddingBottom: '10px', minHeight: 56 }}
        >
          {!onRoot ? (
            <button
              onClick={() => navigate(-1)}
              className="flex items-center gap-1 text-primary font-medium text-sm p-1.5 rounded-lg active:bg-muted"
            >
