    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <aside className="hidden lg:flex fixed inset-y-0 left-0 z-50 w-64 bg-sidebar flex-col">
        <div className="flex items-center gap-3 px-4 py-4 border-b border-sidebar-border bg-slate-950/40">
          ...
          <div className="min-w-0 leading-tight">
            {companyLogo ? <><div className="truncate text-[15px] font-black tracking-[0.02em] text-white">{companyName}</div><div className="mt-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-sky-300">SolarPlan</div></> : <><div className="text-[21px] font-black uppercase tracking-[0.08em] text-white">Lyntra</div><div className="text-[14px] font-black uppercase tracking-[0.12em] text-sky-300">Solutions AB</div></>}
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden lg:ml-64">
        <header className="lg:hidden flex items-center justify-between bg-card/80 backdrop-blur-md border-b border-border" style={{ paddingTop: 'env(safe-area-inset-top)', paddingLeft: 'max(16px, env(safe-area-inset-left))', paddingRight: 'max(16px, env(safe-area-inset-right))', paddingBottom: '10px', minHeight: 56 }}>
          ...
          <div className="flex min-w-0 items-center gap-2"><div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-lg bg-white p-0.5"><img src={logoSrc} alt={`${companyName} logotyp`} className="h-full w-full object-contain" /></div><div className="min-w-0 leading-none"><div className="max-w-36 truncate text-[13px] font-black tracking-[0.02em] text-slate-950">{companyName}</div><div className="mt-1 text-[9px] font-black uppercase tracking-[0.12em] text-sky-700">SolarPlan</div></div></div>
          <div className="w-20" />
        </header>
