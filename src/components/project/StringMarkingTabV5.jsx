...
import { AlertTriangle, Cable, Calculator, CheckCircle2, ChevronDown, ChevronUp, Info, Plus, RefreshCw, Save, Trash2, XCircle } from 'lucide-react';
...
function inverterTopology(product) {
  ...
  if (explicitCounts.length) {
    ...
    // Some imported product rows use strings_per_mppt as total PV inputs. If it is larger than 3 and there are multiple MPPT, treat it as total inputs.
    counts = uniformRaw > 3 && mpptCount > 1 ? distribute(uniformRaw, mpptCount) : Array.from({ length: mpptCount }, () => Math.max(1, Math.round(uniformRaw)));
...
function simulateMppt(panelProduct, inverterProduct, branches, settings) {
  const p = normPanel(panelProduct); const i = normInv(inverterProduct); const valid = (branches || []).filter(b => pos(b.panelCount) > 0);
  if (!p || !i || !valid.length) return null;
  const values = valid.map(branch => ({ ...branch, ...branchElectrical(p, branch.panelCount, settings) }));
  const totalImp = values.reduce((s, x) => s + x.imp, 0), totalIsc = values.reduce((s, x) => s + x.isc, 0), totalPower = values.reduce((s, x) => s + x.power, 0);
  const maxVoc = Math.max(...values.map(x => x.voc)), minVmp = Math.min(...values.map(x => x.vmp)), maxVmp = Math.max(...values.map(x => x.vmp));
...
export default function StringMarkingTabV5({ project, onUpdate, selectedProduct: selectedProductProp }) {
  ...
  const buildPayload = (nextStrings = strings, overrides = {}) => ({ version: 7, source: layout.source, stringCount: overrides.stringCount ?? count, panelProductId: overrides.panelProductId ?? panelId, inverterProductId: activeInverterConfig?.productId || '', inverterConfigs: overrides.inverterConfigs ?? inverterConfigs, selectedInverterConfigId: overrides.selectedInverterConfigId ?? activeInverterId, selectedMppt: overrides.selectedMppt ?? selectedMppt, selectedPv: overrides.selectedPv ?? selectedPvNumber, settings: overrides.settings ?? settings, savedAt: new Date().toISOString(), autosave: true, strings: nextStrings, inverterTopology: (overrides.inverterConfigs ?? inverterConfigs).map(cfg => { const p = inverters.find(item => item.id === cfg.productId); const t = inverterTopology(p); return { inverterConfigId: cfg.id, name: cfg.name, productId: cfg.productId, mpptCount: t.mpptCount, totalPv: t.totalPv, pvPerMppt: t.counts }; }), pvTopology: nextStrings.map(item => ({ stringId: item.id, name: item.name, inverterConfigId: item.inverterConfigId || '', mppt: item.mppt || '', pvInput: item.pvInput || '', panelGroupId: item.panelGroupId || '' })) });
  const persist = async (nextStrings = strings, overrides = {}) => { const payload = buildPayload(nextStrings, overrides); writeStringBackup(project?.id, payload); setSaving(true); setSaveInfo('Sparar...'); try { await onUpdate?.({ string_layout_data: JSON.stringify(payload) }); setSaveInfo(`Sparat ${new Date().toLocaleTimeString('sv-SE')}`); } catch (error) { setSaveInfo('Lokal backup sparad. Servern svarade inte.'); throw error; } finally { setSaving(false); } };
  ...
  const setActiveInverterId = id => { setActiveInverterIdState(id); const product = inverters.find(p => p.id === inverterConfigs.find(cfg => cfg.id === id)?.productId); const t = inverterTopology(product); setSelectedMpptState(1); setSelectedPvState(pvInputsForMppt(1, t)[0]); persist(strings, { selectedInverterConfigId: id, selectedMppt: 1, selectedPv: pvInputsForMppt(1, t)[0] }).catch(() => {}); };
  const updateInverterProduct = (configId, productId) => { const nextConfigs = inverterConfigs.map(cfg => cfg.id === configId ? { ...cfg, productId } : cfg); setInverterConfigs(nextConfigs); setActiveInverterIdState(configId); const t = inverterTopology(inverters.find(p => p.id === productId)); setSelectedMpptState(1); setSelectedPvState(pvInputsForMppt(1, t)[0]); persist(strings, { inverterConfigs: nextConfigs, selectedInverterConfigId: configId, selectedMppt: 1, selectedPv: pvInputsForMppt(1, t)[0] }).catch(() => {}); };
  ...
  const panelMissing = missingPanelFields(panelProduct), invMissing = missingInvFields(activeInverterProduct), hasMissing = panelMissing.length > 0 || invMissing.length > 0;
  ...
  return <div className="space-y-4"><Card className="border-0 shadow-sm"><CardHeader><div className="flex justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Cable className="h-5 w-5 text-primary" />Slingmarkering</CardTitle><p className="text-sm text-muted-foreground">Välj växelriktare, MPPT och fysisk PV-ingång. PV-ingångarna byggs från växelriktarens produktdata.</p></div><div className="flex flex-col items-end gap-2"><Button variant="outline" size="sm" onClick={() => refetch()}><RefreshCw className="mr-2 h-4 w-4" />Uppdatera produkter</Button>{saveInfo && <span className="text-xs text-muted-foreground">{saving ? 'Sparar...' : saveInfo}</span>}</div></div></CardHeader><CardContent className="space-y-4"><div className="grid gap-3 lg:grid-cols-3"><Input label="Antal slingor" min="1" max="80" value={count} onChange={setStringCount} /><ProductSearchSelect label="Solpanel" products={panels} value={panelId} onChange={setPanelId} placeholder="Sök/välj solpanel" /><Select label="Aktiv slinga" value={activeId || ''} onChange={value => { setActiveId(value); const s = strings.find(item => item.id === value); if (s?.inverterConfigId) setActiveInverterIdState(s.inverterConfigId); if (s?.pvInput) setSelectedPv(s.pvInput); else if (s?.mppt) setSelectedMppt(s.mppt); }}>{strings.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></div><InverterManager configs={inverterConfigs} inverters={inverters} activeId={activeInverterId} setActiveId={setActiveInverterId} updateProduct={updateInverterProduct} addInverter={addInverter} removeInverter={removeInverter} /><div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900"><Info className="mr-2 inline h-4 w-4" />Vald växelriktare: <b>{activeInverterConfig?.name}</b>. Produktdata ger <b>{topology.mpptCount} MPPT</b> och <b>{topology.totalPv} PV-ingångar</b>. PV/MPPT: <b>{topology.counts.join(' / ')}</b>. Källa: {topology.source}.</div><div className="grid gap-3 lg:grid-cols-[0.8fr_0.8fr_1.4fr]"><Select label="MPPT-ingång" value={selectedMppt} onChange={setSelectedMppt}>{topology.mppts.map(item => <option key={item.mppt} value={item.mppt}>MPPT {item.mppt}</option>)}</Select><Select label="PV-ingång" value={selectedPvNumber} onChange={setSelectedPv}>{pvInputs.map(pv => <option key={pv} value={pv}>PV {pv}</option>)}</Select><Select label="Panelgrupp för aktiv slinga" value={active?.panelGroupId || ''} onChange={applyPanelGroup}><option value="">Välj panelgrupp...</option>{map.panelGroups.map(group => <option key={group.id} value={group.id}>{group.label} ({group.panelCount} paneler)</option>)}</Select></div><PvInputPanel selectedPv={selectedPvNumber} setSelectedPv={setSelectedPv} pvInputs={pvInputs} strings={strings} selectedMppt={selectedMppt} activeInverterId={activeInverterId} configs={inverterConfigs} onToggleString={toggleStringOnPv} /><div className="grid gap-3 lg:grid-cols-5"><Select label="Väder" value={settings.weather} onChange={value => setSettings({ ...settings, weather: value })}>{Object.keys(WEATHER).map(item => <option key={item}>{item}</option>)}</Select><Select label="Tid" value={settings.timeOfDay} onChange={value => setSettings({ ...settings, timeOfDay: value })}>{Object.keys(TIME).map(item => <option key={item}>{item}</option>)}</Select><Input label="Temperatur °C" value={settings.ambientTemperatureC} onChange={value => setSettings({ ...settings, ambientTemperatureC: Number(value) })} /><Input label="Taklutning °" value={settings.roofTiltDeg} onChange={value => setSettings({ ...settings, roofTiltDeg: Number(value) })} /><Input label="Azimut °" value={settings.roofAzimuthDeg} onChange={value => setSettings({ ...settings, roofAzimuthDeg: Number(value) })} /></div><Canvas map={map} strings={strings.filter(s => !s.inverterConfigId || s.inverterConfigId === activeInverterId)} activeId={activeId} onClickNode={addNode} /><div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-muted/30 p-3"><div className="text-sm text-muted-foreground">Aktiv slinga: <b>{active?.name}</b> · {activeInverterConfig?.name} · PV{active?.pvInput || '-'} · MPPT {active?.mppt || selectedMppt} · {active?.nodes?.length || 0} punkter · {activePanelCount || 0} paneler</div><div className="flex gap-2"><Button variant="outline" className="text-red-600" onClick={clearActiveString}><Trash2 className="mr-2 h-4 w-4" />Rensa slinga</Button><Button onClick={() => persist(strings).catch(() => {})} disabled={saving}><Save className="mr-2 h-4 w-4" />{saving ? 'Sparar...' : 'Spara nu'}</Button></div></div></CardContent></Card><Card className="border-0 shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2"><Calculator className="h-5 w-5 text-primary" />Beräkning för {activeInverterConfig?.name} · MPPT {selectedMppt}</CardTitle></CardHeader><CardContent className="space-y-4">{hasMissing && <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><AlertTriangle className="mr-2 inline h-4 w-4" />Produktdata saknas: {[...panelMissing, ...invMissing].join(', ')}.</div>}{result ? <><div className="flex flex-wrap items-center gap-2"><Badge className={result.status === 'OK' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}>{result.status === 'OK' ? <CheckCircle2 className="mr-1 h-3 w-3" /> : <XCircle className="mr-1 h-3 w-3" />}{result.status}</Badge><span className="text-sm text-muted-foreground">{activeInverterConfig?.name} · MPPT {selectedMppt} · {result.branchCount} slinga/slingor</span></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Effekt" value={round(result.power / 1000, 2)} unit="kW" /><Metric label="Vmp-intervall" value={`${round(result.vmpMin, 1)}-${round(result.vmpMax, 1)}`} unit="V" /><Metric label="Imp total" value={round(result.current, 2)} unit="A" /><Metric label="Isc total" value={round(result.isc, 2)} unit="A" /></div><Button variant="outline" size="sm" onClick={() => setShowDetails(!showDetails)}>{showDetails ? <ChevronUp className="mr-2 h-4 w-4" /> : <ChevronDown className="mr-2 h-4 w-4" />}{showDetails ? 'Dölj mer data' : 'Visa mer data för MPPT/PV'}</Button>{showDetails && <><div className="rounded-xl border bg-background p-3"><div className="mb-2 text-sm font-semibold">Slingor som ingår</div><div className="grid gap-2 lg:grid-cols-2">{result.branchValues.map(branch => <div key={branch.groupId} className="rounded-lg border border-border p-3 text-sm"><div className="font-semibold text-foreground">{branch.label}</div><div className="text-xs text-muted-foreground">{branch.panelCount} paneler i serie · Voc {round(branch.voc, 1)} V · Vmp {round(branch.vmp, 1)} V · Imp {round(branch.imp, 2)} A · Isc {round(branch.isc, 2)} A · Effekt {round(branch.power / 1000, 2)} kW</div></div>)}</div></div><div className="grid gap-2 lg:grid-cols-2">{result.checks.map(check => <CheckRow key={check.label} check={check} />)}</div></>}</> : <div className="rounded-xl border border-muted bg-muted/30 p-4 text-sm text-muted-foreground">Välj solpanel, växelriktare, MPPT, PV-ingång och panelgrupp för att få beräkning.</div>}</CardContent></Card></div>;
}
