...
function createConfig(index, old = {}) {
  return {
    ...
    productSnapshot: old.productSnapshot || null,
  };
}
...
export default function StringMarkingEntry({ project, onUpdate, ...rest }) {
  ...
  useEffect(() => {
    if (!inverters.length || !configs.some(config => config.productId)) return;
  ...
  return (
    <div className="string-inverter-entry relative left-1/2 w-[calc(100vw-2rem)] max-w-[1800px] -translate-x-1/2">
      <style>{`
        .string-inverter-entry .string-inverter-control{position:absolute;right:14px;top:70px;z-index:130;width:302px}
        .string-inverter-entry aside[class*="w-[310px]"]{padding-top:242px!important}
        .string-inverter-entry aside[class*="w-[310px]"]>div.space-y-3>section:nth-of-type(1)>div:first-child>div:first-child{font-size:0}
        .string-inverter-entry aside[class*="w-[310px]"]>div.space-y-3>section:nth-of-type(1)>div:first-child>div:first-child:after{content:'MPPT och PV-ingångar';font-size:.875rem}
        .string-inverter-entry aside[class*="w-[310px]"]>div.space-y-3>section:nth-of-type(1)>div:nth-child(2)>div:first-child{display:none}
        ...
        .string-inverter-entry aside[class*="w-[310px]"]>div.space-y-3>section:nth-of-type(2)>div:first-child>div:first-child:after{content:'Vald PV-ingång';font-size:.875rem}
        .string-inverter-entry aside[class*="w-[310px]"]>div.space-y-3>section:nth-of-type(2)>div:nth-child(2)>div.space-y-2>:nth-child(1),
        .string-inverter-entry aside[class*="w-[310px]"]>div.space-y-3>section:nth-of-type(2)>div:nth-child(2)>div.space-y-2>:nth-child(2){display:none}
        @media(max-width:1279px){.string-inverter-entry .string-inverter-control{right:14px}.string-inverter-entry aside[class*="w-[310px]"]{padding-top:242px!important}}
      `}</style>

      <div className="string-inverter-control rounded-2xl border border-slate-200 bg-white p-3 shadow-xl">
        <div className="mb-2 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 text-sm font-semibold"><Zap className="h-4 w-4 text-orange-500" />Växelriktare</div>
        ...
        <select value={activeConfig?.productId || ''} onChange={event => chooseProduct(event.target.value)} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
        ...
        {activeProduct && <div className={`mt-2 rounded-xl border px-3 py-2 text-[11px] ${spec.complete ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>
          ...
          {!spec.complete && <div className="mt-1 flex gap-1"><AlertTriangle className="h-3.5 w-3.5 shrink-0" />Produktens MPPT/PV-data är ofullständig. Kontrollera antal MPPT och strängar per MPPT i produkten.</div>}
        </div>}
        {status && <div className="mt-1 text-[10px] text-slate-500">{status}</div>}
      </div>

      {anyProductSelected
        ? <StringMarkingTabV7 key={`${project?.id}-${revision}`} project={preparedProject} onUpdate={childUpdate} {...rest} />
        : <div className="min-h-[720px] rounded-[24px] border border-slate-200 bg-slate-100 p-6"><div className="mx-auto mt-28 max-w-lg rounded-2xl border border-amber-200 bg-amber-50 p-5 text-center text-sm text-amber-900"><Zap className="mx-auto mb-3 h-8 w-8" /><div className="font-semibold">Välj växelriktare i Inställningar</div><div className="mt-1">Därefter skapas MPPT och PV-ingångar automatiskt från produktens tekniska data.</div></div></div>}
    </div>
  );
}
