...
function SettingsPortal({ rootRef, children }) {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      const list = root.querySelector('aside[class*="w-[310px]"] > div.space-y-3');
      if (!list) {
...
export default function StringMarkingEntryV2({ project, onUpdate, ...rest }) {
  ...
  return (
    <div ref={rootRef} className={`string-inverter-entry relative left-1/2 w-[calc(100vw-2rem)] max-w-[1800px] -translate-x-1/2 ${anyProductSelected ? 'has-inverter-selected' : 'no-inverter-selected'}`}>
      <style>{`
        ...
        .string-inverter-entry.no-inverter-selected aside[class*="w-[310px]"]>div.space-y-3>section:nth-of-type(2){display:none}
      `}</style>
