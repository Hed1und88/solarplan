  ...
  function renderPanelGroup(layout, group) {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        const panelXM = override ? toNumber(override.xM) : toNumber(group.xM) + col * (s.w + PANEL_GAP_M);
        const panelYM = override ? toNumber(override.yM) : toNumber(group.yM) + row * (s.h + PANEL_GAP_M);
        const shift = previewShift(group, row, col);
        const x = layout.x + panelXM * scale + shift.dx;
        const y = layout.y + panelYM * scale + shift.dy;
    ...
    for (let row = 0; row < rows; row++) {
      ...
      railOffsets.forEach((offset, index) => output.push(<line key={`rail-${group.id}-${row}-${index}`} x1={startX - 8 + groupShift.dx} y1={y + offset} x2={startX + groupWidth + 8 + groupShift.dx} y2={y + offset} stroke="#8b5e34" strokeWidth="4" strokeLinecap="round" onPointerDown={(event) => startDrag(event, 'group', { roofId: layout.roof.id, groupId: group.id })} className="cursor-move" />));
    }
    return output;
  }
...
export default function SolarRoofPlanner({ project, onUpdate }) {
  ...
  const save = (nextRoofs) => { setRoofs(nextRoofs); onUpdate?.({ solar_roof_planner_data: JSON.stringify({ version: 4, scaleType: 'meter', panelModel: DEFAULT_PANEL, roofs: nextRoofs }) }); };
  ...
  const commitGroupMove = (roofId, groupId, dxM, dyM) => updateGroup(roofId, groupId, (group, roof) => ({ ...group, xM: clamp(toNumber(group.xM) + dxM, 0, toNumber(roof.widthM, 8)), yM: clamp(toNumber(group.yM) + dyM, 0, toNumber(roof.roofFallM, 6)) }));
  const commitPanelMove = (roofId, groupId, row, col, dxM, dyM) => updateGroup(roofId, groupId, (group, roof) => { const size = panelSize(group.orientation); const key = `${row}-${col}`; const baseX = toNumber(group.xM) + col * (size.w + PANEL_GAP_M); const baseY = toNumber(group.yM) + row * (size.h + PANEL_GAP_M); const current = group.panelOverrides?.[key] || { xM: baseX, yM: baseY }; return { ...group, panelOverrides: { ...(group.panelOverrides || {}), [key]: { xM: clamp(toNumber(current.xM) + dxM, 0, toNumber(roof.widthM, 8)), yM: clamp(toNumber(current.yM) + dyM, 0, toNumber(roof.roofFallM, 6)) } } }; });
