...
function ScaleRuler({ x, y, scale }) { return <g><line x1={x} y1={y} x2={x + scale} y2={y} stroke="#0f172a" strokeWidth="2" /><line x1={x} y1={y - 6} x2={x} y2={y + 6} stroke="#0f172a" strokeWidth="2" /><line x1={x + scale} y1={y - 6} x2={x + scale} y2={y + 6} stroke="#0f172a" strokeWidth="2" /><text x={x + scale / 2} y={y - 10} textAnchor="middle" fontSize="12" fontWeight="800" fill="#0f172a">1 m</text></g>; }

function groupPhysicalSize(group) {
  const s = panelSize(group.orientation);
...
function getPanelBasePosition(group, row, col) {
  const s = panelSize(group.orientation);
  const key = `${row}-${col}`;
...
function Canvas({ roofs, selectedRoofId, setSelectedRoofId, scale, setScale, deleteRoof, commitPanelMove, commitGroupMove, selectedItem, setSelectedItem, dragMode }) {
  const pad = 80;
  const roofGap = 110;
  ...
  function renderPanelGroup(layout, group) {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        const panelPos = getPanelBasePosition(group, row, col);
...
export default function SolarRoofPlanner({ project, onUpdate }) {
  ...
  const selectedGroupWarnings = (selectedRoof?.panelGroups || []).map((group) => ({ group, size: groupPhysicalSize(group) })).filter(({ group, size }) => size.w + toNumber(group.xM) > toNumber(selectedRoof.widthM) || size.h + toNumber(group.yM) > toNumber(selectedRoof.roofFallM));
  const save = (nextRoofs) => { setRoofs(nextRoofs); onUpdate?.({ solar_roof_planner_data: JSON.stringify({ version: 6, scaleType: 'meter', railMode: 'per-panel', panelModel: DEFAULT_PANEL, roofs: nextRoofs }) }); };
  ...
  const commitGroupMove = (roofId, groupId, dxM, dyM) => updateGroup(roofId, groupId, (group, roof) => { const size = groupPhysicalSize(group); return { ...group, xM: clamp(toNumber(group.xM) + dxM, 0, Math.max(0, toNumber(roof.widthM, 8) - size.w)), yM: clamp(toNumber(group.yM) + dyM, 0, Math.max(0, toNumber(roof.roofFallM, 6) - size.h)) }; });
  const commitPanelMove = (roofId, groupId, row, col, dxM, dyM) => updateGroup(roofId, groupId, (group, roof) => { const size = panelSize(group.orientation); const key = `${row}-${col}`; const current = getPanelBasePosition(group, row, col); return { ...group, panelOverrides: { ...(group.panelOverrides || {}), [key]: { xM: clamp(toNumber(current.xM) + dxM, 0, Math.max(0, toNumber(roof.widthM, 8) - size.w)), yM: clamp(toNumber(current.yM) + dyM, 0, Math.max(0, toNumber(roof.roofFallM, 6) - size.h)) } } }; });
  ...
  return <div className="space-y-4"><Card className="border-0 shadow-sm"><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle className="flex items-center gap-2 text-base"><PanelTop className="h-4 w-4 text-primary" />Solcellskalkylator – fast meterskala</CardTitle><p className="text-sm text-muted-foreground">Bruna skenor följer nu varje panels faktiska position. Den gamla raka gruppskenan är borttagen.</p></div><div className="flex flex-wrap gap-2"><Badge variant="secondary">1 m = {scale} px</Badge><Button variant="outline" size="sm" onClick={() => setDialog('roof')}><Plus className="mr-1 h-4 w-4" />Tak</Button><Button variant="outline" size="sm" onClick={() => setDialog('group')} disabled={!selectedRoof}><PanelTop className="mr-1 h-4 w-4" />Panelgrupp</Button><Button variant="outline" size="sm" onClick={() => setDialog('obstacle')} disabled={!selectedRoof}><AlertTriangle className="mr-1 h-4 w-4" />Hinder</Button></div></div></CardHeader><CardContent className="space-y-4"><div className="grid gap-3 md:grid-cols-5"><div className="rounded-xl bg-muted p-3"><div className="text-xs text-muted-foreground">Paneler</div><div className="text-2xl font-bold">{totals.panels}</div></div><div className="rounded-xl bg-muted p-3"><div className="text-xs text-muted-foreground">Effekt</div><div className="text-2xl font-bold">{totals.kwp.toFixed(2)} kWp</div></div><div className="rounded-xl bg-muted p-3"><div className="text-xs text-muted-foreground">Skenstycken</div><div className="text-2xl font-bold">{totals.rails}</div></div><div className="rounded-xl bg-muted p-3"><div className="text-xs text-muted-foreground">Skenlängd</div><div className="text-2xl font-bold">{totals.len.toFixed(1)} m</div></div><div className="rounded-xl bg-muted p-3"><div className="text-xs text-muted-foreground">Takkrokar</div><div className="text-2xl font-bold">{totals.hooks}</div></div></div><div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-muted/30 p-3"><MousePointer2 className="h-4 w-4 text-primary" /><span className="text-sm font-semibold">Flyttläge:</span><Button size="sm" variant={dragMode === 'panel' ? 'default' : 'outline'} onClick={() => setDragMode('panel')}>En panel</Button><Button size="sm" variant={dragMode === 'group' ? 'default' : 'outline'} onClick={() => setDragMode('group')}>Hela gruppen</Button><span className="text-xs text-muted-foreground">Dubbeltryck på panel försöker också välja grupp.</span></div>{selectedGroupWarnings.length > 0 && <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800"><b>Fysisk kontroll:</b> Minst en panelgrupp är större än vald takyta eller ligger utanför taket. Kontrollera takmått, panelriktning, antal rader/kolumner eller flytta gruppen.</div>}<Canvas roofs={roofs} selectedRoofId={selectedRoofId} setSelectedRoofId={setSelectedRoofId} scale={scale} setScale={setScale} deleteRoof={deleteRoof} commitPanelMove={commitPanelMove} commitGroupMove={commitGroupMove} selectedItem={selectedItem} setSelectedItem={setSelectedItem} dragMode={dragMode} /><div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900"><Home className="mr-2 inline h-4 w-4" />Skenorna ritas panel-för-panel. Flyttas en panel följer dess bruna skenor med. Det finns inte längre någon gammal lång gruppskena ovanpå layouten.</div></CardContent></Card>{dialog === 'roof' && <RoofForm onSave={addRoof} onClose={() => setDialog(null)} />}{dialog === 'group' && <GroupForm onSave={addGroup} onClose={() => setDialog(null)} />}{dialog === 'obstacle' && <ObstacleForm onSave={addObstacle} onClose={() => setDialog(null)} />}</div>;
}