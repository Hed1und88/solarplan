...
function bindControls() {
  const { host, viewport } = parts();
  const scope = viewport || host;
  if (!scope) return;
  const controls = {
    'Zooma in': () => zoom(0.12),
    'Zooma ut': () => zoom(-0.12),
  ...
  Object.entries(controls).forEach(([title, action]) => {
    scope.querySelectorAll(`button[title="${title}"]`).forEach(button => {
...
export default function MapWorkbenchBehaviorController() {
