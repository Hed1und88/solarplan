...
function Result({ value }) {
  ...
  if (value.state === 'blocked') return <div className="mt-3 rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-800"><b>Beräkningen är blockerad</b><ul className="mt-1 list-disc pl-4">{(value.errors || []).map(error => <li key={error}>{error}</li>)}</ul></div>;
  ...
  return <div className="mt-3 space-y-3 rounded-2xl border border-blue-200 bg-blue-50/70 p-3">
    <div className="flex flex-wrap items-center justify-between gap-2"><b className="flex items-center gap-2 text-sm text-blue-950"><Calculator className="h-4 w-4" />Nordmount Parallel</b><Badge className={value.status?.loadsValidated ? 'bg-blue-100 text-blue-800' : 'bg-amber-100 text-amber-800'}>{value.status?.loadsValidated ? 'Laster beräknade' : 'Preliminär last'}</Badge></div>
    <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
      <div className="rounded-xl bg-white p-2.5"><small className="flex items-center gap-1 text-muted-foreground"><Snowflake className="h-3.5 w-3.5" />Snölast · beräknad</small><b>{value.loads?.snow?.designPa ?? '—'} Pa</b></div>
      <div className="rounded-xl bg-white p-2.5"><small className="flex items-center gap-1 text-muted-foreground"><Wind className="h-3.5 w-3.5" />Vind randzon · beräknad</small><b>{value.loads?.wind?.edgePa ?? '—'} Pa</b></div>
      <div className="rounded-xl bg-white p-2.5"><small className="text-muted-foreground">Infästningar · preliminärt</small><div><b>{materials.fastenerCount ?? 0} st</b></div></div>
      <div className="rounded-xl bg-white p-2.5"><small className="text-muted-foreground">Utnyttjande · preliminärt</small><div><b>{value.utilizationPercent ?? 0} %</b></div></div>
    </div>
    <div className="grid gap-2 sm:grid-cols-2">
      <div className="rounded-xl bg-white p-2.5 text-xs"><b>Randzoner</b><div>Gavel {value.zones?.gableM ?? '—'} m · Takfot/nock {value.zones?.eaveRidgeM ?? '—'} m</div></div>
      <div className="rounded-xl bg-white p-2.5 text-xs"><b>Fästavstånd · preliminärt</b><div>{cc.length ? `${Math.min(...cc)}–${Math.max(...cc)} m c/c` : 'Saknas'}</div></div>
    </div>
    <div className="rounded-xl bg-white p-3"><b className="mb-2 flex items-center gap-2 text-xs"><PackageCheck className="h-4 w-4" />Materiallista</b><div className="grid gap-1 text-xs sm:grid-cols-2">{(materials.materials || []).map(item => <div key={item.productId} className="flex justify-between rounded-lg bg-muted/40 px-2 py-1.5"><span>{item.name}</span><b>{item.quantity} {item.unit}</b></div>)}</div></div>
    <div className="rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-950"><b>Kapacitet, automatiskt c/c-val och utnyttjandegrad är preliminära – ej tillverkarverifierade.</b></div>
...
export default function PanelMountingSystemSelector({ project, onUpdate }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setMessage(ready ? 'Validerade Nordmount-laster sparade. Kapacitet/c/c är fortsatt preliminära.' : 'Sparat, men minst ett tak har blockerad eller preliminär last.');
    } finally { setSaving(false); }
  };

  if (!roofs.length) return null;
  return <Card className="border-0 shadow-sm">
    <CardHeader className="flex flex-row items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Wrench className="h-5 w-5 text-primary" />Montagesystem och automatisk dimensionering</CardTitle><p className="mt-1 text-sm text-muted-foreground">Nordmount Parallel använder den validerade snö-/vindmodellen. Kapacitet och c/c visas separat som preliminära.</p></div><Button onClick={save} disabled={saving || !active.every(item => item.system)} className="gap-2"><Save className="h-4 w-4" />{saving ? 'Sparar...' : 'Spara dimensionering'}</Button></CardHeader>
    <CardContent className="space-y-4">
      <div className={`rounded-xl border p-3 text-sm ${ready ? 'border-blue-200 bg-blue-50 text-blue-900' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>{ready ? <CheckCircle2 className="mr-2 inline h-4 w-4" /> : <AlertTriangle className="mr-2 inline h-4 w-4" />}{ready ? 'Snö- och vindlaster beräknade. Kapacitet/c/c är inte tillverkarverifierade.' : 'Välj Nordmount Parallel och fyll i nockhöjd. Cross och ballast blockeras.'}</div>
      {rows.map(item => <div key={item.roof.id} className="rounded-2xl border p-4">
        ...
        <div className="mb-3 flex flex-wrap gap-2 text-xs"><Badge className={item.clamp.hasProductZone ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}>Klämzon: {item.clamp.label}</Badge>{item.engine && <Badge className="bg-blue-100 text-blue-800">Motor: {item.engine.label}</Badge>}</div>
        <ProductSearchSelect label="Montagesystem för detta tak" products={systems} value={item.config.mountingProductId} onChange={value => patchConfig(item.roof, { mountingProductId: value })} placeholder="Välj montagesystem" />
        {item.engine?.id === 'nordmount' && <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          <label className="text-xs">System<select className={INPUT} value={item.config.systemVariant} onChange={event => patchConfig(item.roof, { systemVariant: event.target.value })}><option value="parallel">Parallel</option><option value="cross">Cross – blockerad</option><option value="ballast">Ballast/duktak – blockerad</option></select></label>
          <label className="text-xs">Nockhöjd (m)<input className={INPUT} type="number" min="1" step="0.1" value={item.config.ridgeHeightM} onChange={event => patchConfig(item.roof, { ridgeHeightM: event.target.value })} /></label>
          <label className="text-xs">Terräng<select className={INPUT} value={item.config.terrainCategory} onChange={event => patchConfig(item.roof, { terrainCategory: event.target.value })}><option value="0">0</option><option value="I">I</option><option value="II">II</option><option value="III">III</option><option value="IV">IV</option></select></label>
          <label className="text-xs">Tak/infästning<select className={INPUT} value={item.config.attachmentMethod} onChange={event => patchConfig(item.roof, { attachmentMethod: event.target.value })}><option>Takpannor, Bärläkt/Råspont 2</option><option>Takpannor, Råspont 1</option><option>Papptak</option></select></label>
