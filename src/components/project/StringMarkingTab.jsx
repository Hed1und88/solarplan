...
import { AlertTriangle, Cable, Calculator, CheckCircle2, Circle, Info, RefreshCw, Save, Trash2, XCircle } from 'lucide-react';
...
function simulate(panelProduct, inverterProduct, panelCount, settings) {
  ...
  const checks = [
    ...
    { label: 'MPPT-ström', ok: i.maxa > 0 && p.imp <= i.maxa, nodata: !i.maxa, detail: i.maxa > 0 ? `Imp ${p.imp} A ≤ ${i.maxa} A` : 'Produkten saknar max_input_current_a' },
  ...
  return { status: allOk ? 'OK' : 'Ej godkänd', checks, irr, cell, panelPower, voc, vmp, current: p.imp, isc, power, dcac: i.ac ? power / 1000 / i.ac : 0 };
}
...
export default function StringMarkingTab({ project, onUpdate, selectedProduct: selectedProductProp }) {
  ...
  const activePanelCount = countPanels(draft.length ? draft : active?.nodes || []);
  if (hasMissing) return <div className="space-y-2"><div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><b>Komplettera produktdata för att köra beräkning:</b><ul className="mt-1 list-disc pl-4 space-y-0.5">{[...panelMissing, ...invMissing].map(m => <li key={m}>{m}</li>)}</ul></div><p className="text-xs text-muted-foreground">Redigera produkten under <b>Produkter</b> → skriv in märke + modell → data hämtas automatiskt.</p></div>;
  if (!result) return <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">Markera paneler i slingan för att se beräkning.</div>;
  return <div className="space-y-3"><div className={`rounded-xl border p-3 font-bold text-sm ${result.status === 'OK' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-800'}`}>Status: {result.status}</div><div className="grid grid-cols-2 gap-2 md:grid-cols-4"><Metric label="Paneler" value={activePanelCount} unit="st" /><Metric label="Voc" value={round(result.voc, 1)} unit="V" /><Metric label="Vmp" value={round(result.vmp, 1)} unit="V" /><Metric label="Effekt" value={round(result.power / 1000, 2)} unit="kW" /></div><div className="grid gap-1.5">{result.checks.map(c => <div key={c.label} className={`flex items-start gap-2 rounded-lg px-3 py-2 text-xs ${c.nodata ? 'bg-amber-50 text-amber-700 border border-amber-200' : c.ok ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>{c.nodata ? <Info className="h-4 w-4 shrink-0 mt-0.5" /> : c.ok ? <CheckCircle2 className="h-4 w-4 shrink-0 mt-0.5" /> : <XCircle className="h-4 w-4 shrink-0 mt-0.5" />}<div><b>{c.label}</b>{c.detail && <span className="ml-2 opacity-75">{c.detail}</span>}</div></div>)}</div></div>;
})()}</CardContent></Card><Card><CardHeader><CardTitle className="text-base">Sparade slingor</CardTitle></CardHeader><CardContent className="space-y-2">{strings.map(x => <div key={x.id} className="flex justify-between rounded-xl border p-3 text-sm"><div className="flex items-center gap-2"><Circle className="h-4 w-4" style={{ color: x.color, fill: x.color }} /><div><b>{x.name}</b><div className="text-xs text-muted-foreground">{x.nodes?.length || 0} punkter · {x.panel_count || countPanels(x.nodes)} paneler</div></div></div><Badge variant={x.nodes?.length >= 2 ? 'default' : 'outline'}>{x.nodes?.length >= 2 ? 'Sparad' : 'Ej ritad'}</Badge></div>)}</CardContent></Card></div></div>;
}