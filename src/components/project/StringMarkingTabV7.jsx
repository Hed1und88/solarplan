...
function Canvas({ map, strings, activeId, activeString, tool, scale, setScale, onPanelClick, onStartPolarity, onMovePanel, onMoveGroup }) {
  ...
  return (
    <div ref={scrollRef} className="relative h-[640px] overflow-auto rounded-2xl border border-slate-200 bg-white shadow-inner">
      <div className="sticky left-3 top-3 z-10 flex w-fit gap-2 print:hidden"><button onClick={() => setScale(v => Math.min(120, v + 8))} className="rounded bg-white p-2 shadow"><ZoomIn className="h-4 w-4" /></button><button onClick={() => setScale(v => Math.max(28, v - 8))} className="rounded bg-white p-2 shadow"><ZoomOut className="h-4 w-4" /></button><button onClick={() => setScale(DEFAULT_SCALE)} className="rounded bg-white px-3 py-2 text-xs font-bold shadow">100%</button></div>
      <svg width={map.width} height={map.height} viewBox={`0 0 ${map.width} ${map.height}`} className="block touch-none bg-white" onPointerDown={startPan} onPointerMove={onPointerMove} onPointerUp={endPointer} onPointerCancel={endPointer} onPointerLeave={endPointer}>
        <defs>
...
export default function StringMarkingTabV7({ project, onUpdate }) {
  ...
  const buildPayload = (nextStrings = strings, overrides = {}) => ({ version: 58, source: 'properties_above_inverter_slingor_layout', stringCount: overrides.stringCount ?? countValue, settings: overrides.settings ?? settings, strings: cleanStrings(nextStrings), savedAt: new Date().toISOString() });
  ...
  const propertiesPanel = <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm"><div className="grid gap-3 lg:grid-cols-[1fr_2fr_auto]"><div><div className="text-xs font-black uppercase tracking-wide text-slate-500">Egenskaper</div><div className="mt-1 text-sm font-bold text-slate-900">{active?.name}</div><div className="text-xs text-slate-500">{visibleCount(active)} markerade paneler</div></div><div className="grid gap-2 sm:grid-cols-3"><label className="space-y-1 text-xs font-semibold text-slate-500"><span>Väder</span><select value={settings.weather} onChange={event => patchSettings({ weather: event.target.value })} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900">{WEATHER.map(item => <option key={item} value={item}>{item}</option>)}</select></label><label className="space-y-1 text-xs font-semibold text-slate-500"><span>Tid</span><select value={settings.timeOfDay} onChange={event => patchSettings({ timeOfDay: event.target.value })} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900">{TIMES.map(item => <option key={item} value={item}>{item}</option>)}</select></label><label className="space-y-1 text-xs font-semibold text-slate-500"><span>Temperatur °C</span><input type="number" value={settings.ambientTemperatureC} onChange={event => patchSettings({ ambientTemperatureC: Number(event.target.value) })} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900" /></label></div><div className="flex flex-col gap-2"><Button variant="outline" className="text-red-600" onClick={clearActive}><Trash2 className="mr-2 h-4 w-4" />Rensa</Button><Button onClick={() => persistStrings(strings)} disabled={saving}><Save className="mr-2 h-4 w-4" />{saving ? 'Sparar...' : 'Spara'}</Button></div></div>{saveInfo && <div className="mt-2 text-xs text-slate-500">{saveInfo}</div>}</div>;
  const inverterPanel = <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm"><div className="flex items-center justify-between"><div><div className="text-xs font-black uppercase tracking-wide text-slate-500">Växelriktare</div><div className="mt-1 text-sm font-bold text-slate-900">Växelriktare 1</div><div className="text-xs text-slate-500">Nästa steg: koppla slingor mot MPPT/PV-ingångar.</div></div><div className="rounded-xl border border-blue-100 bg-blue-50 px-3 py-2 text-xs text-blue-900">{strings.length} slingor · {map.panels.length} paneler</div></div></div>;
