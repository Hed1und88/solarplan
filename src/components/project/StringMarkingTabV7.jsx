...
function productLabel(product) {
  return [product?.brand, product?.model].filter(Boolean).join(' ') || product?.name || 'Ingen solpanel vald';
}
...
function parseLayout(project) {
  ...
  if (Array.isArray(planner?.roofs) && planner.roofs.some(roof => (roof.panelGroups || []).length)) return { source: 'solar_roof_planner_data', roofs: planner.roofs, legacy: [] };
  const backup = readLocalJson(plannerStorageKey(project?.id));
  if (Array.isArray(backup?.roofs) && backup.roofs.some(roof => (roof.panelGroups || []).length)) return { source: 'solar_roof_planner_data_backup', roofs: backup.roofs, legacy: [] };
...
function roofPanelProduct(roof, products, fallback) {
  return products.find(product => product.id === roof?.panelProductId) || roof?.panelProductSnapshot || fallback || null;
}
...
function buildMap(layout, products, fallbackPanelProduct) {
  ...
  layout.roofs.forEach((roof, roofIndex) => {
    ...
    (roof.panelGroups || []).forEach((group, groupIndex) => {
      ...
      const size = panelSize(group.orientation, roofProduct);
      ...
      panelGroups.push({
        ...
        panelProduct: roofProduct,
        panelProductId: roofProduct?.id || roof.panelProductId || '',
      });
    });
  });
...
function groupPanelCount(map, groupId) {
  return groupId ? map.panelGroups.find(group => group.id === groupId)?.panelCount || map.panels.filter(panel => panel.groupId === groupId).length : 0;
}

function groupForString(map, string) {
  return map.panelGroups.find(group => group.id === string?.panelGroupId);
}

function panelForString(string, map, products, fallbackPanelProduct) {
  ...
  return products.find(product => product.id === string?.panelProductId)
    || string?.panelProductSnapshot
    || firstSelectedPanel?.panelProduct
    || groupForString(map, string)?.panelProduct
...
function missingPanelFields(product) {
  if (!product) return ['Ingen solpanel vald'];
  return [['voc_v', 'Voc saknas'], ['vmp_v', 'Vmp saknas'], ['isc_a', 'Isc saknas'], ['imp_a', 'Imp saknas'], ['power_watts', 'Effekt saknas']].filter(([key]) => !pos(product[key])).map(([, label]) => label);
}

function missingInvFields(product) {
  if (!product) return ['Ingen växelriktare vald'];
  const missing = [];
  if (!pos(product.max_dc_voltage_v)) missing.push('Max DC-spänning saknas');
  if (!pos(product.startup_voltage_v)) missing.push('Startspänning saknas');
  if (!pos(product.mppt_voltage_min_v) || !pos(product.mppt_voltage_max_v)) missing.push('MPPT-spänningsområde saknas');
  if (!pos(product.max_input_current_a)) missing.push('Max ingångsström saknas');
  if (!pos(product.max_short_circuit_current_a)) missing.push('Max kortslutningsström saknas');
  return missing;
}
...
function branchElectrical(panel, seriesPanelCount, settings) {
  ...
  return {
    irradiance,
    cell,
    voc: panel.voc * (1 + ((cell - 25) * panel.vcoef) / 100) * seriesPanelCount,
    vmp: panel.vmp * (1 + ((cell - 25) * panel.vcoef) / 100) * seriesPanelCount,
    imp: panel.imp,
    isc: panel.isc * (1 + ((cell - 25) * panel.icoef) / 100),
    power: panelPower * seriesPanelCount,
  };
}
...
function simulateMppt(inverterProduct, branches, settings) {
  ...
  return {
    status: checks.filter(check => !check.nodata).every(check => check.ok) ? 'OK' : 'Ej godkänd',
    checks,
    branchValues: values,
...
function Input({ label, value, onChange, min, max }) {
  return (
    <label className="space-y-1 text-xs font-medium text-muted-foreground">
      <span>{label}</span>
      <input type="number" min={min} max={max} value={value ?? ''} onChange={event => onChange(event.target.value)} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" />
    </label>
  );
}
...
function Canvas({ map, strings, activeId, activeString, onClickPanel }) {
  ...
  strings.forEach(string => {
    panelIds(string.nodes || []).forEach(panelId => {
  ...
  return (
    <div className="overflow-auto rounded-2xl border bg-white">
      <svg viewBox={`0 0 ${map.width} ${map.height}`} className="min-h-[560px] w-full min-w-[900px]">
        <defs>
          <pattern id="string-hatch-v7" width="10" height="10" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
            <line x1="0" y1="0" x2="0" y2="10" stroke="#e2e8f0" strokeWidth="3" />
          </pattern>
        </defs>

        {map.roofLayouts.map(r => (
          <g key={r.roof.id || r.roof.name}>
            <text x={r.x} y={r.y - 22} fontSize="18" fontWeight="800">{r.roof.name || 'Tak'}</text>
            <polygon points={roofPoly(r.x, r.y, r.w, r.h, r.roof.shape)} fill="url(#string-hatch-v7)" stroke="#0f172a" strokeWidth="2.5" />
          </g>
        ))}

        {strings.filter(s => s.nodes?.length >= 2).map(s => (
          <g key={s.id}>
            <polyline points={pts(s.nodes)} fill="none" stroke={s.color} strokeWidth={s.id === activeId ? 5 : 3} strokeLinecap="round" strokeLinejoin="round" opacity={s.id === activeId ? 1 : 0.55} />
            {s.nodes.map((node, index) => {
              const p = point(node);
              return p && <circle key={index} cx={p.x} cy={p.y} r={s.id === activeId ? 6 : 4} fill={s.color} stroke="white" />;
            })}
          </g>
        ))}
        ...
        {map.panels.map((panel, index) => {
          ...
          const fill = owner ? `${owner.color || '#2563eb'}22` : '#dbeafe';
          const stroke = isActive ? activeString?.color || '#2563eb' : owner?.color || '#2563eb';
          const strokeWidth = isActive ? 4 : owner ? 3 : 1.5;
          return (
            <g key={panel.id} onClick={() => onClickPanel(panel)} className="cursor-pointer">
              <rect x={panel.x} y={panel.y} width={panel.w} height={panel.h} rx="4" fill={fill} stroke={stroke} strokeWidth={strokeWidth} />
              <text x={panel.x + panel.w / 2} y={panel.y + panel.h / 2 + 4} textAnchor="middle" fontSize="10" fontWeight="800" fill="#1d4ed8">{index + 1}</text>
              {owner && <text x={panel.x + panel.w / 2} y={panel.y + panel.h - 6} textAnchor="middle" fontSize="9" fontWeight="800" fill={owner.color}>{owner.name}</text>}
              <circle cx={panel.black.x} cy={panel.black.y} r="5" fill="#111827" stroke="white" strokeWidth="1.5" pointerEvents="none" />
              <circle cx={panel.red.x} cy={panel.red.y} r="5" fill="#dc2626" stroke="white" strokeWidth="1.5" pointerEvents="none" />
            </g>
          );
        })}
      </svg>
  ...
  return (
    <div className="rounded-xl border border-border p-3">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="text-sm font-semibold text-foreground">Växelriktare på site</div>
        <Button size="sm" variant="outline" onClick={addInverter}><Plus className="mr-2 h-4 w-4" />Lägg till växelriktare</Button>
      </div>
      <div className="grid gap-3 lg:grid-cols-2">
        {configs.map((cfg, index) => {
          const product = inverters.find(p => p.id === cfg.productId);
          const topology = inverterTopology(product);
          return (
            <div key={cfg.id} className={`rounded-xl border p-3 ${cfg.id === activeId ? 'border-primary bg-primary/5' : 'border-border bg-background'}`}>
              <div className="mb-2 flex items-center justify-between gap-2">
                <button type="button" onClick={() => setActiveId(cfg.id)} className="text-left font-bold">{cfg.name || `Växelriktare ${index + 1}`}</button>
                {configs.length > 1 && <Button size="icon" variant="ghost" className="text-red-600" onClick={() => removeInverter(cfg.id)}><Trash2 className="h-4 w-4" /></Button>}
              </div>
              <ProductSearchSelect label="Växelriktare" products={inverters} value={cfg.productId || ''} onChange={value => updateProduct(cfg.id, value)} placeholder="Sök/välj växelriktare" />
              <div className="mt-2 text-xs text-muted-foreground">{inverterLabel(product)} · {topology.mpptCount} MPPT · {topology.totalPv} PV · PV/MPPT: {topology.counts.join(' / ')} · källa: {topology.source}</div>
              {cfg.id !== activeId && <Button size="sm" variant="outline" className="mt-2" onClick={() => setActiveId(cfg.id)}>Välj denna</Button>}
            </div>
          );
        })}
      </div>
...
function PvInputPanel({ selectedPv, setSelectedPv, pvInputs, strings, activeInverterId, configs, onToggleString }) {
  ...
  return (
    <div className="rounded-xl border border-border p-3">
      <div className="mb-2 text-sm font-semibold text-foreground">PV-ingångar</div>
      <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
        {pvInputs.map(pv => {
          const pvStrings = strings.filter(item => item.inverterConfigId === activeInverterId && Number(item.pvInput) === Number(pv));
          return (
            <button key={pv} type="button" onClick={() => setSelectedPv(pv)} className={`rounded-xl border p-3 text-left ${Number(selectedPv) === Number(pv) ? 'border-primary bg-primary/5' : 'border-border bg-background'}`}>
      ...
      <div className="mt-3 rounded-xl bg-muted/40 p-3">
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          {strings.map(item => {
            const checked = item.inverterConfigId === activeInverterId && Number(item.pvInput) === Number(selectedPv);
            const assigned = item.pvInput ? `${cfgName(item.inverterConfigId)} PV${item.pvInput}` : '';
            return (
              <label key={item.id} className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm ${checked ? 'border-primary bg-primary/5' : 'border-border bg-background'}`}>
                <input type="checkbox" checked={checked} onChange={event => onToggleString(item.id, event.target.checked)} />
                <span>{item.name}</span>
                {assigned && !checked && <span className="text-xs text-muted-foreground">{assigned}</span>}
              </label>
            );
          })}
        </div>
...
export default function StringMarkingTabV7({ project, onUpdate, selectedProduct: selectedProductProp }) {
  ...
  const activeString = strings.find(item => item.id === activeId) || strings[0];
  ...
  const [settings, setSettingsState] = useState({
    weather: saved.settings?.weather || 'Soligt',
    timeOfDay: saved.settings?.timeOfDay || '12:00',
    ambientTemperatureC: saved.settings?.ambientTemperatureC ?? 20,
    roofTiltDeg: saved.settings?.roofTiltDeg ?? 27,
    roofAzimuthDeg: saved.settings?.roofAzimuthDeg ?? 180,
  });

  const fallbackPanelProduct = panels.find(product => product.id === fallbackPanelId) || selectedProductProp || null;
  const activeInverterConfig = inverterConfigs.find(cfg => cfg.id === activeInverterId) || inverterConfigs[0];
  const activeInverterProduct = inverters.find(product => product.id === activeInverterConfig?.productId) || null;
  ...
  const activePanelProduct = panelForString(active, map, panels, fallbackPanelProduct);
  ...
  const buildPayload = (nextStrings = strings, overrides = {}) => {
    ...
    return {
      ...
      panelProductMode: 'manual_panel_click',
      stringCount: overrides.stringCount ?? count,
      panelProductId: overrides.panelProductId ?? fallbackPanelId,
      inverterProductId: activeInverterConfig?.productId || '',
      inverterConfigs: nextConfigs,
      selectedInverterConfigId: overrides.selectedInverterConfigId ?? activeInverterId,
      selectedMppt: overrides.selectedMppt ?? selectedMppt,
      selectedPv: overrides.selectedPv ?? selectedPvNumber,
      settings: overrides.settings ?? settings,
      savedAt: new Date().toISOString(),
      autosave: true,
      strings: nextStrings.map(recountString),
      pvTopology: nextStrings.map(item => ({
        stringId: item.id,
        name: item.name,
        ...
        panelCount: countPanels(item.nodes || []) || item.panel_count || 0,
      })),
    };
  };
  ...
  const replaceStrings = (nextStrings, overrides = {}) => {
    const normalized = nextStrings.map(recountString);
    setStrings(normalized);
    persist(normalized, overrides).catch(() => {});
  };
  ...
  const setActiveInverterId = id => {
    ...
    const product = inverters.find(product => product.id === inverterConfigs.find(cfg => cfg.id === id)?.productId);
  ...
  const updateInverterProduct = (configId, productId) => {
    const nextConfigs = inverterConfigs.map(cfg => cfg.id === configId ? { ...cfg, productId } : cfg);
    setInverterConfigs(nextConfigs);
    setActiveInverterIdState(configId);
    const t = inverterTopology(inverters.find(product => product.id === productId));
    const firstPv = pvInputsForMppt(1, t)[0];
    setSelectedMpptState(1);
    setSelectedPvState(firstPv);
    persist(strings, { inverterConfigs: nextConfigs, selectedInverterConfigId: configId, selectedMppt: 1, selectedPv: firstPv }).catch(() => {});
  };
  ...
  const removeInverter = id => {
    ...
    const nextStrings = strings.map(item => item.inverterConfigId === id ? { ...item, inverterConfigId: fallback, inverterProductId: '', pvInput: '', mppt: 1 } : item);
    setInverterConfigs(nextConfigs);
  ...
  const setFallbackPanelId = value => {
    setFallbackPanelIdState(value);
    persist(strings, { panelProductId: value }).catch(() => {});
  };

  const setSettings = nextSettings => {
    setSettingsState(nextSettings);
    persist(strings, { settings: nextSettings }).catch(() => {});
  };

  const setSelectedMppt = value => {
    const nextMppt = Number(value) || 1;
    const firstPv = pvInputsForMppt(nextMppt, topology)[0];
    setSelectedMpptState(nextMppt);
    setSelectedPvState(firstPv);
    persist(strings, { selectedMppt: nextMppt, selectedPv: firstPv }).catch(() => {});
  };

  const setSelectedPv = value => {
    const pv = Number(value) || 1;
    const mppt = mpptFromPvInput(pv, topology);
    setSelectedPvState(pv);
    setSelectedMpptState(mppt);
    persist(strings, { selectedMppt: mppt, selectedPv: pv }).catch(() => {});
  };

  const toggleStringOnPv = (stringId, checked) => {
    const next = strings.map(item => item.id === stringId ? {
      ...item,
      inverterConfigId: checked ? activeInverterId : item.inverterConfigId,
      inverterProductId: checked ? activeInverterConfig?.productId || '' : item.inverterProductId,
      mppt: checked ? selectedMppt : item.mppt,
      pvInput: checked ? selectedPvNumber : '',
    } : item);
    replaceStrings(next);
  ...
  const togglePanelForActiveString = panel => {
    ...
    const panelProduct = group?.panelProduct || panel.panelProduct || activePanelProduct || fallbackPanelProduct;
    ...
    const next = strings.map(item => {
      ...
      const nodes = [...base.nodes, ...nodesForPanel(panel)];
      return recountString({
        ...base,
        inverterConfigId: activeInverterId,
        inverterProductId: activeInverterConfig?.productId || '',
        panelGroupId: base.panelGroupId || panel.groupId,
        panelProductId: panelProduct?.id || '',
        panelProductSnapshot: snapshotProduct(panelProduct),
        nodes,
        mppt: selectedMppt,
        pvInput: selectedPvNumber,
      });
    });
  ...
  const mpptBranches = useMemo(() => strings
    .filter(item => item.inverterConfigId === activeInverterId && Number(item.mppt || mpptFromPvInput(item.pvInput, topology)) === Number(selectedMppt))
    .map(item => ({
      groupId: item.id,
      label: `${item.name}${item.pvInput ? ` · PV${item.pvInput}` : ''}`,
      panelCount: countPanels(item.nodes || []) || item.panel_count || groupPanelCount(map, item.panelGroupId),
      stringId: item.id,
      pvInput: item.pvInput || '',
      panelProduct: panelForString(item, map, panels, fallbackPanelProduct),
    }))
    .filter(branch => branch.panelCount > 0), [strings, activeInverterId, selectedMppt, topology, map, panels, fallbackPanelProduct]);
  ...
  return (
    <div className="space-y-4">
      <Card className="border-0 shadow-sm">
        <CardHeader>
          <div className="flex justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Välj aktiv slinga, MPPT och PV-ingång. Klicka sedan direkt på de paneler som ska ingå i just den slingan. Appen skapar inte längre slingan automatiskt från en hel panelgrupp.</p>
            </div>
            <div className="flex flex-col items-end gap-2">
              <Button variant="outline" size="sm" onClick={() => refetch()}><RefreshCw className="mr-2 h-4 w-4" />Uppdatera produkter</Button>
              {saveInfo && <span className="text-xs text-muted-foreground">{saving ? 'Sparar...' : saveInfo}</span>}
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 lg:grid-cols-3">
            <Input label="Antal slingor" min="1" max="80" value={count} onChange={setStringCount} />
            <ProductSearchSelect label="Reservsolpanel om tak saknar val" products={panels} value={fallbackPanelId} onChange={setFallbackPanelId} placeholder="Sök/välj solpanel" />
            <Select label="Aktiv slinga" value={activeId || ''} onChange={value => { setActiveId(value); const s = strings.find(item => item.id === value); if (s?.inverterConfigId) setActiveInverterIdState(s.inverterConfigId); if (s?.pvInput) setSelectedPv(s.pvInput); else if (s?.mppt) setSelectedMppt(s.mppt); }}>
          ...
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900">
            ...
            Aktiv slinga: <b>{active?.name}</b>. Klicka en panel för att lägga till den. Klicka samma panel igen för att ta bort den från slingan. Om panelen redan låg i en annan slinga flyttas den hit så den inte dubbelräknas.
          </div>

          <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900">
            <Info className="mr-2 inline h-4 w-4" />
            Aktiv slinga använder solpanel: <b>{productLabel(activePanelProduct)}</b>. Panelprodukt hämtas från de paneler du faktiskt markerar.
          </div>
          ...
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900">
            <Info className="mr-2 inline h-4 w-4" />
            Vald växelriktare: <b>{activeInverterConfig?.name}</b>. Produkt/topologidata ger <b>{topology.mpptCount} MPPT</b> och <b>{topology.totalPv} PV-ingångar</b>. PV/MPPT: <b>{topology.counts.join(' / ')}</b>. Källa: {topology.source}.
          </div>

          <div className="grid gap-3 lg:grid-cols-2">
            <Select label="MPPT-ingång" value={selectedMppt} onChange={setSelectedMppt}>
              {topology.mppts.map(item => <option key={item.mppt} value={item.mppt}>MPPT {item.mppt}</option>)}
            </Select>
            <Select label="PV-ingång" value={selectedPvNumber} onChange={setSelectedPv}>
              {pvInputs.map(pv => <option key={pv} value={pv}>PV {pv}</option>)}
            </Select>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-muted/30 p-3">
            <div className="text-sm text-muted-foreground">Aktiv slinga: <b>{active?.name}</b> · {activeInverterConfig?.name} · PV{active?.pvInput || selectedPvNumber || '-'} · MPPT {active?.mppt || selectedMppt} · {activePanelCount || 0} paneler · <b>{productLabel(activePanelProduct)}</b></div>
            <div className="flex gap-2">
              ...
              <Button onClick={() => persist(strings).catch(() => {})} disabled={saving}><Save className="mr-2 h-4 w-4" />{saving ? 'Sparar...' : 'Spara nu'}</Button>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader><CardTitle className="flex items-center gap-2"><Calculator className="h-5 w-5 text-primary" />Beräkning för {activeInverterConfig?.name} · MPPT {selectedMppt}</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          {hasMissing && <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><AlertTriangle className="mr-2 inline h-4 w-4" />Produktdata saknas för aktiv slinga/växelriktare: {[...activePanelMissing, ...invMissing].join(', ')}.</div>}
          {result ? (
            <>
              <div className="flex flex-wrap items-center gap-2">
                <Badge className={result.status === 'OK' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}>{result.status === 'OK' ? <CheckCircle2 className="mr-1 h-3 w-3" /> : <XCircle className="mr-1 h-3 w-3" />}{result.status}</Badge>
                <span className="text-sm text-muted-foreground">{activeInverterConfig?.name} · MPPT {selectedMppt} · {result.branchCount} slinga/slingor</span>
              </div>
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <Metric label="Effekt" value={round(result.power / 1000, 2)} unit="kW" />
                <Metric label="Vmp-intervall" value={`${round(result.vmpMin, 1)}-${round(result.vmpMax, 1)}`} unit="V" />
                <Metric label="Imp total" value={round(result.current, 2)} unit="A" />
                <Metric label="Isc total" value={round(result.isc, 2)} unit="A" />
              </div>
              <div className="grid gap-2 lg:grid-cols-2">{result.checks.map(check => <CheckRow key={check.label} check={check} />)}</div>
              <div className="rounded-xl border bg-background p-3">
                <div className="mb-2 text-sm font-semibold">Slingor som ingår</div>
                <div className="grid gap-2 lg:grid-cols-2">
                  {result.branchValues.map(branch => (
                    <div key={branch.groupId} className="rounded-lg border border-border p-3 text-sm">
                      <div className="font-semibold text-foreground">{branch.label}</div>
                      <div className="text-xs text-muted-foreground">{productLabel(branch.panelProduct)} · {branch.panelCount} paneler i serie · Voc {round(branch.voc, 1)} V · Vmp {round(branch.vmp, 1)} V · Imp {round(branch.imp, 2)} A · Isc {round(branch.isc, 2)} A · Effekt {round(branch.power / 1000, 2)} kW</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="rounded-xl border border-muted bg-muted/30 p-4 text-sm text-muted-foreground">Klicka paneler för aktiv slinga och välj växelriktare, MPPT och PV-ingång för att få beräkning.</div>
          )}
        </CardContent>
      </Card>
    </div>
