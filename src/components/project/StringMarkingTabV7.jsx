      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          panels.push({
            ...
            w: pw,
            h: ph,
            cablePlus: { x: x + pw / 2 - WIRE_OFFSET, y: y + ph * 0.32 },
            cableMinus: { x: x + pw / 2 + WIRE_OFFSET, y: y + ph * 0.72 },
            black: { x, y: y + ph / 2 },
            red: { x: x + pw, y: y + ph / 2 },
            panelProduct,
...
function recount(string) {
  return { ...string, panel_count: panelCount(string.nodes || []), startPolarity: string.startPolarity === 'minus' ? 'minus' : 'plus' };
}
...
function cablePaths(string, map) {
  ...
  const plusPanels = startPolarity === 'plus' ? panels : [...panels].reverse();
  const minusPanels = startPolarity === 'minus' ? panels : [...panels].reverse();
  return {
    panels,
    startPolarity,
    plus: orthogonalPoints(plusPanels, 'cablePlus'),
    minus: orthogonalPoints(minusPanels, 'cableMinus'),
  };
}
...
function Canvas({ map, strings, activeId, activeString, onPanelClick, onStartPolarityChange }) {
  ...
  return <div className="overflow-auto rounded-2xl border bg-white"><svg viewBox={`0 0 ${map.width} ${map.height}`} className="min-h-[560px] w-full min-w-[900px]"><defs><pattern id="roof-hatch" width="10" height="10" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><line x1="0" y1="0" x2="0" y2="10" stroke="#e2e8f0" strokeWidth="3" /></pattern></defs>{map.roofs.map(r => <g key={r.roof.id || r.roof.name}><text x={r.x} y={r.y - 22} fontSize="18" fontWeight="800">{r.roof.name || 'Tak'}</text><polygon points={roofPolygon(r.x, r.y, r.w, r.h, r.roof.shape)} fill="url(#roof-hatch)" stroke="#0f172a" strokeWidth="2.5" /></g>)}{strings.filter(s => panelCount(s.nodes) >= 2).map(s => { const paths = cablePaths(s, map); const selectedPanels = paths.panels; const isActive = s.id === activeId; const isPlusStart = paths.startPolarity === 'plus'; const startPanel = selectedPanels[0]; const endPanel = selectedPanels[selectedPanels.length - 1]; return <g key={s.id}><polyline points={pointString(paths.plus)} fill="none" stroke={PLUS_COLOR} strokeWidth={isActive ? 4 : 2.5} strokeLinecap="round" strokeLinejoin="round" opacity={isActive ? 1 : 0.55} /><polyline points={pointString(paths.minus)} fill="none" stroke={MINUS_COLOR} strokeWidth={isActive ? 4 : 2.5} strokeLinecap="round" strokeLinejoin="round" opacity={isActive ? 0.9 : 0.55} />{isPlusStart && paths.plus.map((pt, i) => <circle key={`p-${i}`} cx={pt.x} cy={pt.y} r={isActive ? 3.5 : 2.5} fill={PLUS_COLOR} stroke="white" />)}{!isPlusStart && paths.minus.map((pt, i) => <circle key={`m-${i}`} cx={pt.x} cy={pt.y} r={isActive ? 3.5 : 2.5} fill={MINUS_COLOR} stroke="white" strokeWidth="1.5" />)}{isActive && <TerminalMarker panel={startPanel} plus color={PLUS_COLOR} selected={isPlusStart} offsetX={-11} onClick={() => onStartPolarityChange?.(s.id, 'plus')} />}{isActive && <TerminalMarker panel={startPanel} plus={false} color={MINUS_COLOR} selected={!isPlusStart} offsetX={11} onClick={() => onStartPolarityChange?.(s.id, 'minus')} />}{isActive && <TerminalMarker panel={endPanel} plus={!isPlusStart} color={!isPlusStart ? PLUS_COLOR : MINUS_COLOR} selected />}</g>; })}{map.panels.map(panel => { const owner = owners.get(panel.id); const selected = activePanels.has(panel.id); const fill = owner ? `${owner.color}22` : '#dbeafe'; const stroke = selected ? activeString?.color || '#2563eb' : owner?.color || '#2563eb'; return <g key={panel.id} onClick={() => onPanelClick(panel)} className="cursor-pointer"><rect x={panel.x} y={panel.y} width={panel.w} height={panel.h} rx="4" fill={fill} stroke={stroke} strokeWidth={selected ? 4 : owner ? 3 : 1.5} /><text x={panel.x + panel.w / 2} y={panel.y + panel.h / 2 + 4} textAnchor="middle" fontSize="10" fontWeight="800" fill="#1d4ed8">{panel.number}</text>{owner && <text x={panel.x + panel.w / 2} y={panel.y + panel.h - 6} textAnchor="middle" fontSize="9" fontWeight="800" fill={owner.color}>{owner.name}</text>}<circle cx={panel.black.x} cy={panel.black.y} r="5" fill="#111827" stroke="white" strokeWidth="1.5" /><circle cx={panel.red.x} cy={panel.red.y} r="5" fill="#dc2626" stroke="white" strokeWidth="1.5" /></g>; })}</svg></div>;
}
...
export default function StringMarkingTabV7({ project, onUpdate, selectedProduct: selectedProductProp }) {
  ...
  const setStartPolarity = (stringId, polarity) => {
    const nextPolarity = polarity === 'minus' ? 'minus' : 'plus';
    replaceStrings(strings.map(s => s.id === stringId ? recount({ ...s, startPolarity: nextPolarity }) : s));
  };
  ...
  return <div className="space-y-4">
    <Card className="border-0 shadow-sm"><CardHeader><div className="flex justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Cable className="h-5 w-5 text-primary" />Slingmarkering</CardTitle><p className="text-sm text-muted-foreground">Klicka + eller - vid första panelen för att välja vilken pol som är start. Endast startledaren visar punkter. Motsatt ledare följer kabelvägen utan punkter och ansluter vid sista panelen.</p></div><div className="flex flex-col items-end gap-2"><Button variant="outline" size="sm" onClick={() => refetch()}><RefreshCw className="mr-2 h-4 w-4" />Uppdatera produkter</Button>{saveInfo && <span className="text-xs text-muted-foreground">{saving ? 'Sparar...' : saveInfo}</span>}</div></div></CardHeader><CardContent className="space-y-4">
      ...
      <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900"><Info className="mr-2 inline h-4 w-4" />Välj aktiv slinga, klicka panelerna i kabelordning och välj startpol genom att trycka på + eller - vid första panelen.</div>
      ...
      <Canvas map={map} strings={strings} activeId={activeId} activeString={active} onPanelClick={togglePanel} onStartPolarityChange={setStartPolarity} />
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-muted/30 p-3"><div className="text-sm text-muted-foreground">Aktiv slinga: <b>{active?.name}</b> - {activeCount} paneler - start {active?.startPolarity === 'minus' ? '-' : '+'} - MPPT {selectedMppt} - PV{selectedPvSafe}</div><div className="flex gap-2"><Button variant="outline" className="text-red-600" onClick={clearActive}><Trash2 className="mr-2 h-4 w-4" />Rensa slinga</Button><Button onClick={() => persist(strings)} disabled={saving}><Save className="mr-2 h-4 w-4" />{saving ? 'Sparar...' : 'Spara nu'}</Button></div></div>
    </CardContent></Card>
