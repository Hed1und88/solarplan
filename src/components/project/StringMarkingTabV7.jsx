    ...
    (roof.panelGroups || []).forEach((group, groupIndex) => {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          panels.push({ id: `r${roofId}-g${group.id || groupIndex}-${row}-${col}`, roofId, groupId, groupName, x, y, w: pw, h: ph, panelProduct: roofProduct, black: { x, y: y + ph / 2 }, red: { x: x + pw, y: y + ph / 2 } });
        }
      }
      panelGroups.push({ id: groupId, label: `${roof.name || 'Tak'} / ${groupName}`, roofName: roof.name || 'Tak', groupName, panelCount: rows * cols, panelProduct: roofProduct, panelProductId: roofProduct?.id || roof.panelProductId || '' });
    });
  });
...
function makeString(i, old = {}, fallbackInverterId = '') {
  ...
  return { id: old.id || uid(), name: old.name || `Slinga ${i + 1}`, color: old.color || COLORS[i % COLORS.length], nodes: old.nodes || [], panel_count: old.panel_count || 0, panelGroupId: old.panelGroupId || '', panelProductId: old.panelProductId || '', panelProductSnapshot: old.panelProductSnapshot || null, inverterConfigId: old.inverterConfigId || fallbackInverterId, inverterProductId: old.inverterProductId || '', mppt: old.mppt || 1, pvInput: legacyPvInput || '', ...old, pvInput: legacyPvInput || old.pvInput || '' };
}
...
function groupForString(map, string) { return map.panelGroups.find(group => group.id === string?.panelGroupId); }
...
function normPanel(product) { return product && { pmax: pos(product.power_watts), voc: pos(product.voc_v), vmp: pos(product.vmp_v), isc: pos(product.isc_a), imp: pos(product.imp_a), pcoef: num(product.temp_coeff_pmax_percent_c, -0.35), vcoef: num(product.temp_coeff_voc_percent_c, -0.27), icoef: num(product.temp_coeff_isc_percent_c, 0.05), noct: pos(product.noct_c, 45) }; }
function normInv(product) { const ac = pos(product?.power_watts) / 1000; return product && { ac, maxdc: pos(product.max_dc_power_kw, ac * 1.5), maxv: pos(product.max_dc_voltage_v), start: pos(product.startup_voltage_v), mpptmin: pos(product.mppt_voltage_min_v), mpptmax: pos(product.mppt_voltage_max_v), maxa: pos(product.max_input_current_a), maxisc: pos(product.max_short_circuit_current_a) }; }
...
function Select({ label, value, onChange, children }) { return <label className="space-y-1 text-xs font-medium text-muted-foreground"><span>{label}</span><select value={value ?? ''} onChange={event => onChange(event.target.value)} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground">{children}</select></label>; }
function Input({ label, value, onChange, min, max }) { return <label className="space-y-1 text-xs font-medium text-muted-foreground"><span>{label}</span><input type="number" min={min} max={max} value={value ?? ''} onChange={event => onChange(event.target.value)} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" /></label>; }
...
function Canvas({ map, strings, activeId, onClickNode }) {
  ...
  return <div className="overflow-auto rounded-2xl border bg-white"><svg viewBox={`0 0 ${map.width} ${map.height}`} className="min-h-[560px] w-full min-w-[900px]"><defs><pattern id="string-hatch-v7" width="10" height="10" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><line x1="0" y1="0" x2="0" y2="10" stroke="#e2e8f0" strokeWidth="3" /></pattern></defs>{map.roofLayouts.map(r => <g key={r.roof.id || r.roof.name}><text x={r.x} y={r.y - 22} fontSize="18" fontWeight="800">{r.roof.name || 'Tak'}</text><polygon points={roofPoly(r.x, r.y, r.w, r.h, r.roof.shape)} fill="url(#string-hatch-v7)" stroke="#0f172a" strokeWidth="2.5" /></g>)}{map.panels.map((panel, index) => <g key={panel.id}><rect x={panel.x} y={panel.y} width={panel.w} height={panel.h} rx="4" fill="#dbeafe" stroke="#2563eb" /><text x={panel.x + panel.w / 2} y={panel.y + panel.h / 2 + 4} textAnchor="middle" fontSize="10" fontWeight="800" fill="#1d4ed8">{index + 1}</text><circle cx={panel.black.x} cy={panel.black.y} r="7" fill="#111827" stroke="white" strokeWidth="2" onClick={() => onClickNode(panel, 'black')} className="cursor-pointer" /><circle cx={panel.red.x} cy={panel.red.y} r="7" fill="#dc2626" stroke="white" strokeWidth="2" onClick={() => onClickNode(panel, 'red')} className="cursor-pointer" /></g>)}{strings.filter(s => s.nodes?.length >= 2).map(s => <g key={s.id}><polyline points={pts(s.nodes)} fill="none" stroke={s.color} strokeWidth={s.id === activeId ? 5 : 3} strokeLinecap="round" strokeLinejoin="round" />{s.nodes.map((node, index) => { const p = point(node); return p && <circle key={index} cx={p.x} cy={p.y} r={s.id === activeId ? 6 : 4} fill={s.color} stroke="white" />; })}</g>)}</svg></div>;
}
...
export default function StringMarkingTabV7({ project, onUpdate, selectedProduct: selectedProductProp }) {
  ...
  const buildPayload = (nextStrings = strings, overrides = {}) => {
    ...
    return {
      ...
      panelProductMode: 'per_roof_panel_group',
      ...
      autosave: true,
      strings: nextStrings,
      pvTopology: nextStrings.map(item => ({ stringId: item.id, name: item.name, inverterConfigId: item.inverterConfigId || '', mppt: item.mppt || '', pvInput: item.pvInput || '', panelGroupId: item.panelGroupId || '', panelProductId: item.panelProductId || '' })),
    };
  };
  ...
  const setActiveInverterId = id => { setActiveInverterIdState(id); const product = inverters.find(product => product.id === inverterConfigs.find(cfg => cfg.id === id)?.productId); const t = inverterTopology(product); const firstPv = pvInputsForMppt(1, t)[0]; setSelectedMpptState(1); setSelectedPvState(firstPv); persist(strings, { selectedInverterConfigId: id, selectedMppt: 1, selectedPv: firstPv }).catch(() => {}); };
  const updateInverterProduct = (configId, productId) => { const nextConfigs = inverterConfigs.map(cfg => cfg.id === configId ? { ...cfg, productId } : cfg); setInverterConfigs(nextConfigs); setActiveInverterIdState(configId); const t = inverterTopology(inverters.find(product => product.id === productId)); const firstPv = pvInputsForMppt(1, t)[0]; setSelectedMpptState(1); setSelectedPvState(firstPv); persist(strings, { inverterConfigs: nextConfigs, selectedInverterConfigId: configId, selectedMppt: 1, selectedPv: firstPv }).catch(() => {}); };
  ...
  const setStringCount = value => { const nextCount = Math.max(1, Math.min(80, Number(value) || 1)); const next = Array.from({ length: nextCount }, (_, index) => makeString(index, strings[index], activeInverterId)); setCountState(nextCount); setStrings(next); if (!next.some(item => item.id === activeId)) setActiveId(next[0]?.id || null); persist(next, { stringCount: nextCount }).catch(() => {}); };
  ...
  const applyPanelGroup = groupId => {
    const group = map.panelGroups.find(item => item.id === groupId);
    const panelProduct = group?.panelProduct || fallbackPanelProduct;
    const nodes = nodesForGroup(map, groupId);
    patchActiveString({ inverterConfigId: activeInverterId, inverterProductId: activeInverterConfig?.productId || '', panelGroupId: groupId, nodes, panel_count: countPanels(nodes), panelProductId: panelProduct?.id || '', panelProductSnapshot: snapshotProduct(panelProduct), mppt: selectedMppt, pvInput: selectedPvNumber });
  };
  const addNode = (panel, terminal) => {
    ...
    patchActiveString({ inverterConfigId: activeInverterId, inverterProductId: activeInverterConfig?.productId || '', panelGroupId: active.panelGroupId || panel.groupId, panelProductId: panelProduct?.id || '', panelProductSnapshot: snapshotProduct(panelProduct), nodes, panel_count: countPanels(nodes), mppt: selectedMppt, pvInput: selectedPvNumber });
  };
  ...
  const mpptBranches = useMemo(() => strings
    ...
    .map(item => ({ groupId: item.id, label: `${item.name}${item.pvInput ? ` · PV${item.pvInput}` : ''}`, panelCount: countPanels(item.nodes || []) || item.panel_count || groupPanelCount(map, item.panelGroupId), stringId: item.id, pvInput: item.pvInput || '', panelProduct: panelForString(item, map, panels, fallbackPanelProduct) }))
  ...
  return <div className="space-y-4"><Card className="border-0 shadow-sm"><CardHeader><div className="flex justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Cable className="h-5 w-5 text-primary" />Slingmarkering</CardTitle><p className="text-sm text-muted-foreground">Solpanelen hämtas automatiskt från valt tak/panelgrupp. Exempel: Tak 2 / Panelgrupp 5 använder solpanelen som är vald på Tak 2.</p></div><div className="flex flex-col items-end gap-2"><Button variant="outline" size="sm" onClick={() => refetch()}><RefreshCw className="mr-2 h-4 w-4" />Uppdatera produkter</Button>{saveInfo && <span className="text-xs text-muted-foreground">{saving ? 'Sparar...' : saveInfo}</span>}</div></div></CardHeader><CardContent className="space-y-4">
    ...
    <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900"><Info className="mr-2 inline h-4 w-4" />Aktiv slinga använder solpanel: <b>{productLabel(activePanelProduct)}</b>. Den väljs automatiskt från panelgruppen/taket.</div>
