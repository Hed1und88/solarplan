...
import { AlertTriangle, Cable, Calculator, CheckCircle2, Info, Link2, RefreshCw, Save, Trash2, XCircle } from 'lucide-react';
...
function stored(project) {
  ...
  if (d?.version >= 2 && Array.isArray(d.strings)) return d;
...
function groupPanelCount(map, groupId) {
  if (!groupId) return 0;
  const fromGroup = map.panelGroups.find(group => group.id === groupId)?.panelCount;
  return fromGroup || map.panels.filter(panel => panel.groupId === groupId).length;
}
...
function simulateParallelMppt(panelProduct, inverterProduct, branches, settings) {
  ...
  const branchValues = validBranches.map(branch => ({
    ...branch,
    ...branchElectrical(p, branch.panelCount, settings),
  }));
  ...
  return {
    status: checks.every(check => check.ok) ? 'OK' : 'Ej godkänd',
    checks,
    branchValues,
    branchCount,
    irr: branchValues[0]?.irr || 0,
    cell: branchValues[0]?.cell || 0,
    voc: maxVoc,
    vmpMin: minVmp,
    vmpMax: maxVmp,
    current: totalImp,
    isc: totalIsc,
    power: totalPower,
    dcac: i.ac ? totalPower / 1000 / i.ac : 0,
  };
}
...
function ParallelGroupPicker({ groups, active, strings, value, onChange }) {
  ...
  const setGroup = (groupId, checked) => {
    ...
    const ordered = groups.map(group => group.id).filter(groupId => next.has(groupId));
    onChange(ordered);
  };
  ...
  return (
    <div className="rounded-xl border border-border p-3">
      <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-foreground">
        <Link2 className="h-4 w-4 text-primary" /> Parallellkoppla panelgrupper på vald MPPT
      </div>
      {!activeGroupId ? (
        <div className="rounded-lg bg-amber-50 p-3 text-xs text-amber-800">Välj först panelgruppen som den aktiva slingan tillhör.</div>
      ) : options.length === 0 ? (
        <div className="rounded-lg bg-muted/50 p-3 text-xs text-muted-foreground">Det finns ingen annan panelgrupp att parallellkoppla med.</div>
      ) : (
        <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
          {options.map(group => {
            ...
            return (
              <label key={group.id} className={`flex cursor-pointer items-start gap-2 rounded-lg border px-3 py-2 text-sm ${checked ? 'border-primary bg-primary/5' : 'border-border bg-background'}`}>
                <input type="checkbox" className="mt-1" checked={checked} onChange={e => setGroup(group.id, e.target.checked)} />
                <span className="min-w-0">
                  <span className="block font-medium text-foreground">{group.label}</span>
                  <span className="block text-xs text-muted-foreground">{group.panelCount} paneler{linkedString ? ` · ${linkedString.name}` : ' · räknas från panelgruppen'}</span>
                </span>
              </label>
            );
          })}
      ...
      <p className="mt-2 text-xs text-muted-foreground">Exempel: välj Panelgrupp 1 som aktiv slinga och bocka i Panelgrupp 2. Då räknas båda som parallella strängar på samma MPPT.</p>
    </div>
  );
}
...
function CheckRow({ check }) {
  ...
  return (
    <div className={`flex items-start gap-2 rounded-lg border p-2 text-sm ${tone}`}>
      <Icon className="mt-0.5 h-4 w-4 shrink-0" />
      <div>
        <div className="font-semibold">{check.label}</div>
        <div className="text-xs opacity-90">{check.detail}</div>
      </div>
    </div>
  );
}

export default function StringMarkingTabV2({ project, onUpdate, selectedProduct: selectedProductProp }) {
  ...
  const activeNodes = draft.length ? draft : active?.nodes || [];
  ...
  const branches = useMemo(() => {
    if (!active) return [];
    const groupIds = active.panelGroupId
      ? [...new Set([active.panelGroupId, ...selectedParallelGroupIds])]
      : [];

    if (!groupIds.length && activePanelCount > 0) {
      return [{ groupId: active.id, label: active.name, panelCount: activePanelCount, stringId: active.id, hasMappedString: true }];
    }
    ...
    return groupIds.map(groupId => {
      ...
      const nodes = linkedString?.id === active.id ? activeNodes : (linkedString?.nodes || []);
      const panelCount = countPanels(nodes) || linkedString?.panel_count || groupPanelCount(map, groupId);
      return {
        groupId,
        label: group?.label || linkedString?.name || groupId,
        panelCount,
        stringId: linkedString?.id || '',
        hasMappedString: Boolean(linkedString),
      };
    }).filter(branch => branch.panelCount > 0);
  }, [active, activeNodes, activePanelCount, map, selectedParallelGroupIds, strings]);

  const result = simulateParallelMppt(panelProduct, invProduct, branches, settings);
  ...
  const applyPanelGroupToDraft = groupId => {
    ...
    setDraft(nodes);
    updateActiveString({ panelGroupId: groupId, parallelPanelGroupIds: [], parallelEnabled: false, parallelGroupId: '' });
  };

  const updateParallelPanelGroups = groupIds => {
    const clean = groupIds.filter(groupId => groupId && groupId !== active?.panelGroupId);
    updateActiveString({
      parallelPanelGroupIds: clean,
      parallelEnabled: clean.length > 0,
      parallelGroupId: clean.length ? `${active?.panelGroupId || active?.id || 'manual'}__${clean.join('__')}` : '',
    });
  };
    ...
    try {
      await onUpdate({
        string_layout_data: JSON.stringify({
          ...
          settings,
          strings: next,
          parallelTopology: next.map(item => ({
            stringId: item.id,
            name: item.name,
            panelGroupId: item.panelGroupId || '',
            mppt: item.mppt || 1,
            parallelPanelGroupIds: item.parallelPanelGroupIds || [],
          })),
        }),
      });
    } finally {
  ...
  const clearActiveString = () => {
    const next = strings.map(item => item.id === active?.id ? { ...item, nodes: [], panel_count: 0, parallelPanelGroupIds: [], parallelEnabled: false, parallelGroupId: '' } : item);
    setStrings(next);
    setDraft([]);
  };
  ...
  return (
    <div className="space-y-4">
      <Card className="border-0 shadow-sm">
        <CardHeader>
          <div className="flex justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Välj panelgrupp, MPPT och vilka panelgrupper som är parallellkopplade med den aktiva slingan. Beräkningen summerar ström och kortslutningsström mot växelriktarens MPPT-värden.</p>
            </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900">
            <Info className="mr-2 inline h-4 w-4" />Svart = minus. Röd = plus. När du väljer en panelgrupp fylls slingan med gruppens paneler automatiskt. Parallellkopplade panelgrupper räknas som flera strängar på samma MPPT.
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-muted/30 p-3">
            <div className="text-sm text-muted-foreground">
              Aktiv ritning: <b>{active?.name}</b> · {draft.length || active?.nodes?.length || 0} punkter · {activePanelCount || 0} paneler · MPPT {active?.mppt || 1}
              {selectedParallelGroupIds.length > 0 && <span> · parallell med {selectedParallelGroupIds.length} panelgrupp{selectedParallelGroupIds.length === 1 ? '' : 'er'}</span>}
            </div>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Calculator className="h-5 w-5 text-primary" />Beräkning för vald MPPT</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {hasMissing && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
              <AlertTriangle className="mr-2 inline h-4 w-4" />Produktdata saknas: {[...panelMissing, ...invMissing].join(', ')}.
            </div>
          )}
          ...
          {result ? (
            <>
              <div className="flex flex-wrap items-center gap-2">
                <Badge className={result.status === 'OK' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}>
                  {result.status === 'OK' ? <CheckCircle2 className="mr-1 h-3 w-3" /> : <XCircle className="mr-1 h-3 w-3" />}{result.status}
                </Badge>
                <span className="text-sm text-muted-foreground">MPPT {active?.mppt || 1} · {result.branchCount} parallell{result.branchCount === 1 ? ' sträng' : 'a strängar'}</span>
              </div>
              ...
              <div className="rounded-xl border bg-background p-3">
                <div className="mb-2 text-sm font-semibold">Parallellkopplade grenar som ingår i beräkningen</div>
                <div className="grid gap-2 lg:grid-cols-2">
                  {result.branchValues.map(branch => (
                    <div key={branch.groupId} className="rounded-lg border border-border p-3 text-sm">
                      ...
                      <div className="text-xs text-muted-foreground">{branch.panelCount} paneler i serie · Vmp {round(branch.vmp, 1)} V · Imp {round(branch.imp, 2)} A · Isc {round(branch.isc, 2)} A</div>
                      {!branch.hasMappedString && <div className="mt-1 text-xs text-amber-700">Ingen separat slinga ritad ännu. Panelantal hämtas direkt från panelgruppen.</div>}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid gap-2 lg:grid-cols-2">
                {result.checks.map(check => <CheckRow key={check.label} check={check} />)}
              </div>
            </>
          ) : (
            <div className="rounded-xl border border-muted bg-muted/30 p-4 text-sm text-muted-foreground">Välj solpanel, växelriktare och panelgrupp för att få beräkning.</div>
          )}
        </CardContent>
      </Card>
    </div>
