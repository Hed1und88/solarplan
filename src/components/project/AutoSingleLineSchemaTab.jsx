...
function normalizeSingleLine(project, products) {
  ...
  const inverters = inverterConfigs.map((cfg, index) => {
    const product = products.find(item => item.id === cfg.productId) || null;
    const inverterStrings = strings
      .filter(item => item.inverterConfigId === cfg.id || (!item.inverterConfigId && index === 0))
      .map(item => {
        const panelProduct = products.find(product => product.id === item.panelProductId) || item.panelProductSnapshot || null;
...
export default function AutoSingleLineSchemaTab({ project, onUpdate, products = [] }) {
  ...
  const exportSvg = () => {
    ...
    a.href = url;
    a.download = `enlinjeschema-${project?.name || 'projekt'}.svg`;
    a.click();
