...
import { AlertTriangle, Cable, Calculator, CheckCircle2, ChevronDown, ChevronUp, Info, Link2, RefreshCw, Save, Trash2, XCircle } from 'lucide-react';
...
function parseLayout(project) {
  ...
  if (legacy.length) {
    return { source: 'panel_layout_data', roofs: [{ id: 'legacy', name: 'Panelritning', widthM: pos(old?.roofWidth, pos(project?.roof_width_m, 8)), roofFallM: pos(old?.roofHeight, pos(project?.roof_height_m, 6)), shape: 'Rektangel' }], legacy };
  }
...
function makeString(i, old = {}) {
  const parallelPanelGroupIds = Array.isArray(old.parallelPanelGroupIds) ? old.parallelPanelGroupIds : [];
  return {
    id: old.id || uid(),
    ...
    mppt: old.mppt || 1,
    parallelEnabled: Boolean(old.parallelEnabled || parallelPanelGroupIds.length),
    parallelGroupId: old.parallelGroupId || '',
    parallelPanelGroupIds,
    ...old,
  };
}
...
function missingInvFields(p) {
  ...
  if (!pos(p.strings_per_mppt)) m.push('Antal strängar per MPPT saknas');
  return m;
}
...
function simulate(panelProduct, inverterProduct, branches, settings) {
  ...
  const branchCount = branchValues.length;
  ...
  const voltageSpreadPct = minVmp > 0 ? ((maxVmp - minVmp) / minVmp) * 100 : 0;
  const panelCountSpread = new Set(branchValues.map(item => item.panelCount)).size;
  ...
  const checks = [
    ...
    { label: 'MPPT-ström', ok: i.maxa > 0 && totalImp <= i.maxa, nodata: !i.maxa, detail: i.maxa > 0 ? `Total Imp ${round(totalImp, 2)} A (${branchCount} parallell) ≤ ${i.maxa} A` : 'Produkten saknar max_input_current_a' },
    ...
    { label: 'Strängar per MPPT', ok: i.stringsPerMppt > 0 && branchCount <= i.stringsPerMppt, nodata: !i.stringsPerMppt, detail: `${branchCount} av ${i.stringsPerMppt} tillåtna parallella strängar på MPPT` },
    { label: 'Parallell balans', ok: !isParallel || (panelCountSpread === 1 && voltageSpreadPct <= 2), nodata: false, detail: isParallel ? `Vmp-spread ${round(voltageSpreadPct, 2)}%. Parallella grupper bör ha samma panelantal och likvärdig spänning.` : 'Endast en sträng på MPPT' },
  ...
  return { status: checks.every(check => check.ok) ? 'OK' : 'Ej godkänd', checks, branchValues, branchCount, irr: branchValues[0]?.irr || 0, cell: branchValues[0]?.cell || 0, voc: maxVoc, vmpMin: minVmp, vmpMax: maxVmp, current: totalImp, isc: totalIsc, power: totalPower };
}
  ...
  return (
    <div className="overflow-auto rounded-2xl border bg-white">
      <svg viewBox={`0 0 ${map.width} ${map.height}`} className="min-h-[560px] w-full min-w-[900px]">
        <defs><pattern id="string-hatch-v3" width="10" height="10" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><line x1="0" y1="0" x2="0" y2="10" stroke="#e2e8f0" strokeWidth="3" /></pattern></defs>
        {map.roofLayouts.map(r => <g key={r.roof.id || r.roof.name}><text x={r.x} y={r.y - 22} fontSize="18" fontWeight="800">{r.roof.name || 'Tak'}</text><polygon points={roofPoly(r.x, r.y, r.w, r.h, r.roof.shape)} fill="url(#string-hatch-v3)" stroke="#0f172a" strokeWidth="2.5" /></g>)}
...
function ParallelGroupPicker({ groups, active, strings, value, onChange }) {
  const activeGroupId = active?.panelGroupId || '';
  const selected = new Set(Array.isArray(value) ? value : []);
  const options = groups.filter(group => group.id !== activeGroupId);
  const setGroup = (groupId, checked) => {
    const next = new Set(selected);
    if (checked) next.add(groupId); else next.delete(groupId);
    onChange(groups.map(group => group.id).filter(groupId => next.has(groupId)));
  };
  return (
    <div className="rounded-xl border border-border p-3">
      <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-foreground"><Link2 className="h-4 w-4 text-primary" /> Parallellkoppla panelgrupper på vald MPPT</div>
      {!activeGroupId ? <div className="rounded-lg bg-amber-50 p-3 text-xs text-amber-800">Välj först panelgruppen som den aktiva slingan tillhör.</div> : (
        <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
          {options.map(group => {
            const linkedString = strings.find(item => item.panelGroupId === group.id);
            const checked = selected.has(group.id);
            return <label key={group.id} className={`flex cursor-pointer items-start gap-2 rounded-lg border px-3 py-2 text-sm ${checked ? 'border-primary bg-primary/5' : 'border-border bg-background'}`}><input type="checkbox" className="mt-1" checked={checked} onChange={e => setGroup(group.id, e.target.checked)} /><span className="min-w-0"><span className="block font-medium text-foreground">{group.label}</span><span className="block text-xs text-muted-foreground">{group.panelCount} paneler{linkedString ? ` · ${linkedString.name}` : ' · räknas från panelgruppen'}</span></span></label>;
          })}
        </div>
      )}
      <p className="mt-2 text-xs text-muted-foreground">Ändringen sparas direkt när du bockar i eller ur en panelgrupp.</p>
    </div>
  );
}

export default function StringMarkingTabV3({ project, onUpdate, selectedProduct: selectedProductProp }) {
  ...
  const selectedParallelGroupIds = Array.isArray(active?.parallelPanelGroupIds) ? active.parallelPanelGroupIds.filter(groupId => groupId && groupId !== active?.panelGroupId) : [];
  ...
  const buildPayload = (nextStrings = strings, overrides = {}) => ({
    ...
    autosave: true,
    strings: nextStrings,
    parallelTopology: nextStrings.map(item => ({ stringId: item.id, name: item.name, panelGroupId: item.panelGroupId || '', mppt: item.mppt || 1, parallelPanelGroupIds: item.parallelPanelGroupIds || [] })),
  });
  ...
  const patchActiveString = (patch) => {
    const next = strings.map(item => item.id === active?.id ? { ...item, ...patch } : item);
    replaceStrings(next);
  };
  ...
  const applyPanelGroup = groupId => {
    ...
    patchActiveString({ panelGroupId: groupId, nodes, panel_count: countPanels(nodes), parallelPanelGroupIds: [], parallelEnabled: false, parallelGroupId: '' });
  };

  const updateParallelPanelGroups = groupIds => {
    const clean = groupIds.filter(groupId => groupId && groupId !== active?.panelGroupId);
    patchActiveString({ parallelPanelGroupIds: clean, parallelEnabled: clean.length > 0, parallelGroupId: clean.length ? `${active?.panelGroupId || active?.id || 'manual'}__${clean.join('__')}` : '' });
  };

  const addNode = (panel, terminal) => {
    if (!active?.id) return;
    const nodes = [...(active.nodes || []), { panelId: panel.id, terminal }];
    patchActiveString({ nodes, panel_count: countPanels(nodes) });
  };

  const clearActiveString = () => patchActiveString({ nodes: [], panel_count: 0, panelGroupId: '', parallelPanelGroupIds: [], parallelEnabled: false, parallelGroupId: '' });

  const branches = useMemo(() => {
    if (!active) return [];
    const groupIds = active.panelGroupId ? [...new Set([active.panelGroupId, ...selectedParallelGroupIds])] : [];
    if (!groupIds.length && activePanelCount > 0) return [{ groupId: active.id, label: active.name, panelCount: activePanelCount, stringId: active.id, hasMappedString: true }];
    return groupIds.map(groupId => {
      const group = map.panelGroups.find(item => item.id === groupId);
      const linkedString = groupId === active.panelGroupId ? active : strings.find(item => item.panelGroupId === groupId);
      const panelCount = countPanels(linkedString?.nodes || []) || linkedString?.panel_count || groupPanelCount(map, groupId);
      return { groupId, label: group?.label || linkedString?.name || groupId, panelCount, stringId: linkedString?.id || '', hasMappedString: Boolean(linkedString) };
    }).filter(branch => branch.panelCount > 0);
  }, [active, activePanelCount, map, selectedParallelGroupIds, strings]);

  const result = simulate(panelProduct, invProduct, branches, settings);
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader>
          <div className="flex justify-between gap-3">
            <div><CardTitle className="flex items-center gap-2"><Cable className="h-5 w-5 text-primary" />Slingmarkering</CardTitle><p className="text-sm text-muted-foreground">Välj panelgrupp och MPPT. Slingan sparas direkt när du väljer panelgrupp, byter MPPT, parallellkopplar eller klickar i ritningen.</p></div>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 lg:grid-cols-4"><Input label="Antal slingor" min="1" max="20" value={count} onChange={setStringCount} /><ProductSearchSelect label="Solpanel" products={panels} value={panelId} onChange={setPanelId} placeholder="Sök/välj solpanel" /><ProductSearchSelect label="Växelriktare" products={inverters} value={invId} onChange={setInvId} placeholder="Sök/välj växelriktare" /><Select label="Aktiv slinga" value={activeId || ''} onChange={setActiveId}>{strings.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></div>
          <div className="grid gap-3 lg:grid-cols-[1.4fr_0.8fr]"><Select label="Panelgrupp från Paneler" value={active?.panelGroupId || ''} onChange={applyPanelGroup}><option value="">Välj panelgrupp...</option>{map.panelGroups.map(group => <option key={group.id} value={group.id}>{group.label} ({group.panelCount} paneler)</option>)}</Select><Select label="MPPT-ingång" value={active?.mppt || 1} onChange={value => patchActiveString({ mppt: Number(value) || 1 })}>{Array.from({ length: inverter?.mpptCount || 4 }, (_, i) => <option key={i + 1} value={i + 1}>MPPT {i + 1}</option>)}</Select></div>
          <ParallelGroupPicker groups={map.panelGroups} active={active} strings={strings} value={selectedParallelGroupIds} onChange={updateParallelPanelGroups} />
          ...
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900"><Info className="mr-2 inline h-4 w-4" />Svart = minus. Röd = plus. När du väljer en panelgrupp fylls slingan och sparas automatiskt.</div>
          <Canvas map={map} strings={strings} activeId={activeId} onClickNode={addNode} />
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-muted/30 p-3"><div className="text-sm text-muted-foreground">Aktiv ritning: <b>{active?.name}</b> · {active?.nodes?.length || 0} punkter · {activePanelCount || 0} paneler · MPPT {active?.mppt || 1}{selectedParallelGroupIds.length > 0 && <span> · parallell med {selectedParallelGroupIds.length} panelgrupp{selectedParallelGroupIds.length === 1 ? '' : 'er'}</span>}</div><div className="flex gap-2"><Button variant="outline" className="text-red-600" onClick={clearActiveString}><Trash2 className="mr-2 h-4 w-4" />Rensa slinga</Button><Button onClick={() => persist(strings).catch(() => {})} disabled={saving}><Save className="mr-2 h-4 w-4" />{saving ? 'Sparar...' : 'Spara nu'}</Button></div></div>
        </CardContent>
      </Card>

      <Card className="border-0 shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2"><Calculator className="h-5 w-5 text-primary" />Beräkning för vald MPPT</CardTitle></CardHeader><CardContent className="space-y-4">
        ...
        {result ? <><div className="flex flex-wrap items-center gap-2"><Badge className={result.status === 'OK' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}>{result.status === 'OK' ? <CheckCircle2 className="mr-1 h-3 w-3" /> : <XCircle className="mr-1 h-3 w-3" />}{result.status}</Badge><span className="text-sm text-muted-foreground">MPPT {active?.mppt || 1} · {result.branchCount} parallell{result.branchCount === 1 ? ' sträng' : 'a strängar'}</span></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Effekt" value={round(result.power / 1000, 2)} unit="kW" /><Metric label="Vmp-intervall" value={`${round(result.vmpMin, 1)}-${round(result.vmpMax, 1)}`} unit="V" /><Metric label="Imp total" value={round(result.current, 2)} unit="A" /><Metric label="Isc total" value={round(result.isc, 2)} unit="A" /></div><Button variant="outline" size="sm" onClick={() => setShowDetails(!showDetails)}>{showDetails ? <ChevronUp className="mr-2 h-4 w-4" /> : <ChevronDown className="mr-2 h-4 w-4" />}{showDetails ? 'Dölj mer data' : 'Visa mer data för slingan'}</Button>{showDetails && <><div className="rounded-xl border bg-background p-3"><div className="mb-2 text-sm font-semibold">Parallellkopplade grenar som ingår i beräkningen</div><div className="grid gap-2 lg:grid-cols-2">{result.branchValues.map(branch => <div key={branch.groupId} className="rounded-lg border border-border p-3 text-sm"><div className="font-semibold text-foreground">{branch.label}</div><div className="text-xs text-muted-foreground">{branch.panelCount} paneler i serie · Voc {round(branch.voc, 1)} V · Vmp {round(branch.vmp, 1)} V · Imp {round(branch.imp, 2)} A · Isc {round(branch.isc, 2)} A · Effekt {round(branch.power / 1000, 2)} kW</div>{!branch.hasMappedString && <div className="mt-1 text-xs text-amber-700">Ingen separat slinga ritad ännu. Panelantal hämtas direkt från panelgruppen.</div>}</div>)}</div></div><div className="grid gap-2 lg:grid-cols-2">{result.checks.map(check => <CheckRow key={check.label} check={check} />)}</div></>}</> : <div className="rounded-xl border border-muted bg-muted/30 p-4 text-sm text-muted-foreground">Välj solpanel, växelriktare och panelgrupp för att få beräkning.</div>}
      </CardContent></Card>
    </div>
  );
}
