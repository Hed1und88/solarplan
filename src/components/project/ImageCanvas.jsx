...
import { Upload, Camera, RotateCcw, Move, ZoomIn, ZoomOut } from 'lucide-react';
...
export default function ImageCanvas({ imageUrl, items, onItemsChange, itemRenderer, onImageUpload, label }) {
  ...
  const handleFileChange = async (e) => {
    ...
    if (file) {
      ...
      reader.onload = (ev) => {
        onImageUpload(ev.target.result, file);
      };
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    onItemsChange(items.map(item => 
      item.id === dragging ? { ...item, x: clampedX, y: clampedY } : item
    ));
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        ref={canvasRef}
        className="relative rounded-xl overflow-hidden shadow-lg bg-black cursor-crosshair select-none"
